...
同一个开关也是隧道详情页顶部的一键式「停止自动化」开关，让你在查看隧道时就能直接压制自动化。全局「禁止系统休眠」设置（默认关闭）会在有隧道连接时让计算机保持唤醒（Windows、macOS、Linux 三平台均生效），最后一条隧道断开后立即释放，使空闲或休眠的电源策略不会悄悄杀掉隧道。当另一个客户端已占用某条隧道的地址时，WireGuide Plus 现在会弹出一个交互式对话框，点名冲突的软件并主动提供「停止该隧道自动连接」——由你决定，它绝不强退另一个客户端。
