...
- **托盘连接状态通知** — 启动后 15 秒（等提权弹窗落定后）弹出全部隧道总览气泡：逐条列出实时状态与自动/手动模式，按可调时长自动关闭（默认 10 秒，可在设置中调整）；
  之后网络变化（切换 Wi-Fi、拔网线、断网……）导致隧道状态改变时，弹出按隧道逐行显示的状态气泡
  （「🟢 Connected: 名称」绿 /「🔴 Disconnected: 名称」红 / 🟡 连接中琥珀），浮在其他应用之上但
  不抢焦点，**常驻直到你处理**：手动关闭、打开主窗口、点断开，或被更新的事件替换。气泡停靠在屏幕工作区角落、不压主窗口内容，按住可拖到任意位置。三平台行为一致（macOS/Linux 此前没有弹窗）。
- **统一的托盘操作** — 左键或右键单击都打开托盘菜单，左键双击打开主窗口，三平台一致；托盘菜单项在 macOS/Linux 带 🟢 / 🔴 状态符号（Windows 菜单由 GDI 绘制、无法渲染彩色 emoji，改用 ✔ / ✗）。
...
4. **系统集成** — 开机自启、启动最小化（Windows 最小化到任务栏 / macOS 与 Linux
   最小化到托盘）、托盘连接状态通知（启动后 15 秒全部隧道总览，停留时长可调；状态变化气泡
   不抢焦点、常驻直到处理）、统一的托盘点击（单击出菜单、双击开主窗口）。
5. **窗口标题与交互细节** 等更多改进。
...
- **SSID 未知时失败关闭**：如果当前网络的 SSID 暂时未知——Wi-Fi 接口抖动后先恢复、身份报告尚未跟上（同一 SSID 下重新配置网络的场景）——以 SSID 为条件的规则按**无法判定**处理，该隧道本轮跳过。默认状态绝不会悄悄压过引擎尚无法评估的条件；SSID 重新已知的瞬间，评估随即恢复。
...
日志查看器工具栏与设置页的日志区都提供「打开日志目录」按钮，可在系统文件管理器中直接打开该目录。
