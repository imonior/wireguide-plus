<script>
  ...
  function onResizeKeydown(e) {
    // Keyboard support for the separator so it works without a mouse.
    if (e.key === 'ArrowLeft') {
      e.preventDefault();
      listPaneWidth.set(Math.max(LIST_PANE_MIN, $listPaneWidth - 24));
      saveListPrefs();
    } else if (e.key === 'ArrowRight') {
      e.preventDefault();
      listPaneWidth.set(Math.min(LIST_PANE_MAX, $listPaneWidth + 24));
      saveListPrefs();
      ...
      {#if currentView === 'tunnels'}
        <div class="tunnels-view">
          ...
          <div class="pane-resizer" class:resizing
            on:mousedown={onResizeStart} on:dblclick={onResizeReset} on:keydown={onResizeKeydown}
            role="separator" aria-orientation="vertical" tabindex="0" title="Drag to resize · double-click to reset"></div>
  ...
  {#if showZipResult}
    <div class="modal-backdrop" on:click={() => showZipResult = false}>
      <div class="modal modal-zip-result" on:click|stopPropagation
        role="dialog" aria-modal="true" tabindex="-1" aria-labelledby="zip-result-title">
