...
- **「更新をサイレントでインストール」設定（設定 → アップデート）** — オンにすると、ダウンロードした更新がインストーラー UI を表示せずにインストールされ、完了後にアプリが再起動します。デフォルトはオフです。
- **Windows の更新はデフォルトで対話型インストーラーを使用** — 「今すぐ更新」でダウンロード完了後に通常のインストールと同じインストーラーが起動し、ウィザードで確認してインストールできます。完了ページ（「実行」チェックボックス既定オン）で新バージョンが起動します。

### 🐛 修正

- **Linux：更新完了後にアプリを自動起動** — deb/rpm パッケージマネージャーによる更新後、アプリが自動的に再起動され新しいバージョンが読み込まれます（従来は手動で開く必要がありました）。

> macOS は 1.4.0 から導入されたアプリバンドルのインプレース更新を継続します（/Applications への書き込み時にシステムの認証プロンプトが表示され、完了後に自動再起動します）。
