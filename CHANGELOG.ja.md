...
- **macOS のステータスバブルを macOS 自身に配置させました。** Wails は macOS でプライマリディスプレイを一切報告しないため、バブルはメインウィンドウ基準のフォールバックに静かに落ちていました。しかもその座標はポイントとピクセルが混在していて、結果は画面中央付近。角丸のカードは四角いウィンドウに収められていたので、四隅が硬い枠として見えていました。いまはサイズ・位置・角丸クリップ・最前面レベル・Space／フルスクリーン追従まで、`NSScreen.visibleFrame` を読むネイティブ処理で統一しています。
- **macOS のバブルを再びドラッグできます。** Wails の JS ウィンドウドラッグは macOS では一度も作動しておらず、バブルをつかんでも反応がありませんでした。現在はネイティブのマウスイベントモニタが動かします。フォーカスはそのまま（アプリのアクティベーションもキーボードフォーカスの奪取も起きず）、移動閾値未満のクリックはボタンにそのまま届きます。
- **2 つ目の起動による「切断／再接続」バブル嵐を解消しました。** 2 つの GUI インスタンスは 1 つの特権ヘルパーを共有しています。終了側のインスタンスはトンネルを畳んでヘルパーも停止しますが、生き残り側はまだそれに依存しています。その健康モニタはヘルパーがクラッシュしたと判断し、管理者権限のダイアログを出して再インストールに入り、自動化がトンネルを再接続 — メインウィンドウは接続中のまま、ポップアップだけ「切断」を告げるという矛盾が起きていました。GUI はプロセスの生存期間全体で単一インスタンスの排他ロックを保持し、2 つ目の起動は共有状態に触れる前に実行中インスタンスへウィンドウ表示を要求して終了します。

### 🔧 変更

- **macOS/Linux のステータスバブルは作業領域の右上に停まります。**メニューバーの直下、macOS 自身が通知を置く角です（2.4.0 の右下は Dock に重なっていました）。Windows は従来どおりトレイ側の角に停まります。
...
- **同一 SSID のままのネットワーク切替でも自動化ルールが守られます。** Wi-Fi ネットワークの再設定（静的 IP → DHCP など）ではインターフェースが一瞬途切れ、SSID の再報告より先にアドレスだけ復帰することがあります。その隙間では SSID 条件の切断ルールが（一時的に空の）SSID に一致せず、トンネルのデフォルト状態が黙って上書きし、ルールが切断状態を保つべきトンネルが接続されてしまいました。macOS には定期ポーリングの保険がないため、誤接続が残り続けることもあります。エンジンは不完全な情報に対してフェイルクローズで動作します：ルールが SSID に依存するトンネルは、そのネットワークの SSID が不明な間ラウンドごとスキップされ、デフォルト状態がエンジンが判定できない条件に代わって判断することはありません。SSID が再び判明した瞬間に判断も再開します — 新たな SSID 報告そのものが評価を起動し、実行・スキップの直後には数秒後に一度きりのセトル再チェックも控えています。再接続モニタのポリシーゲートも同じ不完全な情報を拒否するため、ネットワーク揺らぎ時の盲復旧がエンジンなら下さない決定を行うことはなくなりました。
- **macOS のアイコンチェーンを補完し、新しい角丸アートを配布物の全てに反映。** メニューバーのトレイアイコン、アプリバンドルアイコン（Dock / Launchpad）、DMG インストーラのアイコンが従来の角四角のアートのままだったものを、Windows の角丸 `.ico` も同じ単一ソース `build/appicon.png` から生成するようにしました。
...
- **ログビューアからログフォルダをワンクリックで開く** — ツールバーに「ログフォルダを開く」ボタンを追加。Finder / エクスプローラー / システムのファイルマネージャーでログの保存フォルダをそのまま開きます（設定のログ項目と同じ操作）。
- **トレイメニューの状態記号** — 接続中トンネルは 🟢、未接続は 🔴 を名前と併記（macOS/Linux。Windows のトレイメニューは GDI 描画でカラー絵文字を使えないため単色の ✔ / ✗）。
- **3プラットフォーム共通のトレイクリック操作** — シングルクリック（左でも右でも）でトレイメニューを開き、左ダブルクリックでメインウィンドウを開きます。

### 🔧 変更

- **状態通知がトンネルごと1行表示に（全プラットフォーム統一）：**「🟢 Connected: 名前」/「🔴 Disconnected: 名前」/ 接続中は 🟡、行全体を状態で色分け。従来の連結キャプションは廃止。
- **状態バブルは操作するまで表示を維持** — 自動閉じるタイマーを廃止。✕ で閉じる、メインウィンドウを開く、切断を押す、または新しいイベントに置き換わるまで残ります。アプリから離れている間に切断警告が10秒で消えて見逃されることはありません。通知時間は起動時 overview バブルにのみ適用されます。
- **macOS のバブルはフォーカスを奪わず最前面に** — アプリをアクティベートせずにウィンドウレベルを上げて他アプリ（フルスクリーンのアプリでも）の上に浮かびます。通知は今やっていることの*上*に出るもので、*置き換える*ものではないからです。

### 🐛 修正

- **ネットワークの瞬断時に自動化ポリシーに反する再接続が発生しなくなりました** — NICが一瞬落ちる操作（例：固定IP設定時）で、切断検知モニターがキャッシュ済みトンネルを無条件に全回復し、トンネル単位の自動化ルールより先に接続してしまうことがありました。両方の回復経路が同じポリシーを先に照会し、切断ルール・未識別ネットワーク・一時停止/除外中のトンネルには触れません。

## [2.3.10] - 2026-09-24

### ✨ 追加

- **アドレス競合時は、ユーザーが判断するまでそのトンネルの自動化を一時停止します。**自動接続が他のクライアントに占有済みのアドレスと衝突すると、衝突元のソフトウェア名とそのトンネルの自動化状態を表示する常駐ダイアログが現れ、選択肢は「このトンネルの自動化を停止」と「再試行を継続」の二つだけです。「後で処理する」経路は存在しません：回答するまで helper はそのトンネルの自動接続を保留し、未処理の衝突は GUI 起動時および helper 再起動後に再表示されます。
- **起動時オーバービューバブル。**GUI 起動の 15 秒後、全トンネルの最新状態と自動/手動モードを一覧表示するポップアップが表示されます（従来の起動 10 秒後の状態バブルを置き換え）。

### 🐛 修正

- **接続試行の失敗による「切断された」ゴーストバブルがなくなりました。**実際に接続されていたトンネルのみが切断として通知されるようになり、「接続中→失敗」は誤検知されず、真の再接続は引き続き通知されます。

### 🔧 変更

- **状態バブルは全プラットフォームで統一された視覚記号を採用：**接続中＝緑のチェック＋緑文字、切断＝赤の×＋赤文字、 接続試行中＝黄色のドット。Windows ネイティブバブルではベクター描画で表現し、macOS/Linux バブルや起動時オーバービューの各行も同じ語彙です。
...
- `tools/checkrelease` を追加：5 つの i18n ロケール JSON が同一のキー集合を公開していること、および 5 つの CHANGELOG すべてに今回のバージョンのセクションが含まれていることを検証する読み取り専用のガード。`bump:version` は先にこれを実行するため、翻訳のズレや更新履歴の欠落を含んだリリースを防げます。

### 🔧 変更

- 統一されたローカル品質ゲート（`task check`：`go vet` + `./internal/...` 向け競合テスト）を追加し、CI を `main` への直接プッシュ時にも実行するよう変更。`main` へのプッシュが一切テストされない空白を閉じました。
- CI が Wails bindings を再生成し、コミット済みの `frontend/bindings` との差分があれば失敗するようになり、フロントエンド/バックエンド間の IPC 呼び出しの静かな不一致を防ぎます。

## [2.3.7] - 2026-09-23

### 🐛 修正

- **Linux の「システムのスリープを禁止」が再び実際に機能するように**: ローカルの編集で `systemd-inhibit` の呼び出しが誤って削除され、抑制ロックを持たない空回りのシェルのみが残っていたため、この設定は Linux 上で静かに何もしない状態でした。自動監視シェルを `systemd-inhibit` で包む形に戻し、オーバーライド ロックが再び効くようにしました。
- **プロキシの変更（更新チェック用）がリアルタイムにブロードキャストされるように**: `proxy_mode` / `proxy_url` はこれまで `settings_changed` ペイロードに含まれておらず、設定 UI のプロキシ反映コードが事実上死んでいて、他クライアントや CLI からの変更が反映されませんでした。両フィールドをペイロードに加え、変更時に送信するようにしました。
- **設定保存を堅牢化しフィールドのリセットを防止**: 保存パスが最新の設定を読み込んでから画面項目のみを上書きする（展開）ようになり、新しく追加した設定項目が古い保存パスによってゼロ値で書き戻されることはなくなりました。これは 2.3.6 の「保存されない」症状の根本原因を仕組み上解消するものです。

### 🔧 変更

- **リポジトリ全体の gofmt 整形**（動作変更なし）。
...
- **「システムのスリープを防止」が macOS と Linux でも動作**: 従来は Windows のみ有効（`SetThreadExecutionState` の呼び出し）で、他のプラットフォームでは何もしない実装でした。macOS は `caffeinate` のアサーションを、Linux は `systemd-inhibit` のロックを保持するようになり、3 プラットフォームすべてでネットワークアダプターの電源が維持され、バックグラウンドのトンネルが生き残ります。

### 🔧 変更

- **スリープ防止はトンネル接続中のみ有効に**: この設定をオンにしても、アプリの稼働中ずっとマシンを起こしたままにすることはなくなりました。helper（特権バックグラウンドデーモン。GUI を閉じても動作し続けます）は、最初のトンネルが接続状態になった時点でスリープ防止を有効にし、最後のトンネルが切断されると即座に解除します。設定の説明文も全言語で更新しました。
- **トンネルごとの自動化スイッチを自動化エディターへ移動**: 「自動化から除外」チェックボックスはトンネルポリシーパネルから離れ、自動化エディターの先頭に「自動化機能のオン/オフ」スイッチとして配置されました（詳細ビューのトグルと並立）。両方のコントロールは共通の read-modify-write ヘルパー経由で同じフラグを読み書きし、エディターを閉じた際に再読み込みするため、状態が食い違うことはありません。

### 🐛 修正

- **「システムのスリープを防止」が保存されない**: 設定画面の保存ペイロードにこの項目が含まれておらず、再起動するとスイッチが暗黙のうちにオフへ戻り、次回起動時にスリープ防止が復元されませんでした。
- **Linux でオフにするたびに `sleep infinity` プロセスが漏洩**: `systemd-inhibit` 本体だけを終了させており、抑制ロックを保持するために実行していたコマンドが取り残されて毎回孤児プロセスになっていました。プロセスグループごと終了させるようにし、helper がクラッシュした場合も子プロセスを回収します。
- **スリープ防止の子プロセスが落ちても「守られている」と誤認**: 有効化する前に生存確認を行うようにし、`caffeinate` / `systemd-inhibit` が終了済みなら再起動します。黙って保護されないままになることはありません。

## [2.3.3] - 2026-09-22

### ✨ 追加

- **トンネル詳細ビューに「自動化を停止」トグルを追加**: 設定画面の「自動化から除外」と同じ働きを持つ目立つワンクリックスイッチです。オンにすると、あなたがオフにするまでエンジンはそのトンネルの接続/切断を行いません。
- **グローバルな「システムのスリープを防止」（バックグラウンド维持）**: アプ起動中に PC を起覺状態に保ち、アイドル/スリープの電源ポリシーがトンネルを靜かに切るのを防ぐ新しい任意設定です。仞定はオフ——あなたが明示的にオンにしない限り、システムの電源ポリシーは上書きされません。Windows では `SetThreadExecutionState` を呼び出します。
- **対話型アドレス競合ダイアログ**: 別の VPN クライアントがトンネルのアドレスを既に佔有している場合、競合しているソフトウェアとアダプターを明示し、そのトンネルの自動接続を停止する選択能を提示します。決定するのはあなた——他のクライアントを強制停止することはありません。

### 🐛 修正

- **既に接続されているトンネルを状態トレイが再通知していた問題**: 接続トーストは状態が実際に変化したトンネル（接続/切断）だけを通知するようになり、常時接続のトンネルは一度だけ通知され、30 秒ごとのステータスポーリンで「X 接続済み」と繰り返しポップアップしなくなりました。これが「TS453Dmini 接続済み」が繰り返した根本原因です——実際にチャタリングしていたのは別のトンネルで、トーストはその時点でオンラインだったトンネルの名前を報じていただけでした。
- **手動の「自動化を停止」がすべての自動再接続を上書きするように**: 死活接続モニターも各トンネルの除外設定を尊重するようになり、トンネルの自動化を無効にすると本当に自動再接続されなくなります（以前はハンドシェイクタイムアウト時にモニターが再接続する可能性がありました）。

### 🔧 変更

- **旧の署名ベースのトースト重複除去を削除**: これは単にバブルを隐すだけで、本当の接続変化を規い陛していました。現在はソースで本当のトンネル別の状態変化を通知することでノイズを減らしています。

## [2.3.2] - 2026-09-21

### ✨ 追加

- **自動化からトンネルを除外**: トンネルのポリシーパネルに「自動化から除外」スイッチを追加しました。有効にするとエンジンはそのトンネルを一切接続・切断せず、ルールとデフォルト状態は維持されます。同じトンネルを別の WireGuard クライアントも管理している場合に使います。手動オーバーライドは再起動までしか効かないのに対し、このスイッチは永続します。

### 🐛 修正

- **自動化が 30 秒ごとにトンネルへ連続アクセスし、状態トレイを連打していた**: 接続が失敗すると—最も多い原因は別の WireGuard クライアントが既にそのトンネルの `Address` を占有していることです（Windows は `The object already exists` を報告）—レコンサイラはポーリングのたびにバックオフなしで再試行し、Wintun アダプターを繰り返し作り直し、毎回接続トーストを再表示していました。失敗した自動化接続は現在バックオフ付きで再試行され（30 秒 → 1 分 → 2 分 → 5 分、上限あり）、トンネルがアクティブになったり手動ラッチが設定されたりネットワーク規則が変わったりすると解除されます。
- **同じ状態で「接続しました」トーストが繰り返し表示されていた**: トレイは現在、状態通知を署名で重複除去します。既に通知済み（または直前に抑制された）状態は再表示せず、初回の抑制は再トーストする代わりに Info としてログに記録されます。
- **接続がアドレス割り当ての段階で不可解に失敗していた**: `AssignAddress` は現在、「アドレスが既にこのインターフェース上にある」ことを成功（冪等）とみなします。アドレスが本当に奪われている場合は、エラーに競合するアダプターの名前を明記し、同じトンネルを実行している別のクライアントが何れか分かるようにします。
- **起動時に同じトンネルが別の場所で動いていることを警告するように**: 起動直後に helper が各トンネルの `Address` をマシンの実インターフェースと照合し（自身のアダプターは除外）、別のクライアントが既に占有している場合は競合するアダプターの名前とサービス停止のヒントをログに記録します。

### 📝 ドキュメント

- README に「1 つのトンネルに 1 つのクライアント」というルールと、トンネルごとの「自動化から除外」スイッチの説明を追加しました。
## [2.3.1] - 2026-09-21

### ✨ 追加

- **画面オフ・ロック・スリープ中も接続を維持**：新設定「アイドル時も接続を維持」（既定でオン、トンネル単位で上書き可）を追加しました。接続時に PersistentKeepalive を強制するため、画面オフ・スクリーンセーバー・ロックでも静かに切断されません。スリープ後は復帰時に自動で再検出して再接続します。Windows ではシステム スリープに加え、ディスプレイ電源とセッションのロック/解除も監視するようになり、スクリーンセーバーやロックが死んだトンネルを残すことがなくなりました。

### 🐛 修正

- **WebView2 が入っている環境で「未インストール」と誤判定（Windows）**：起動時チェックが 64 ビット レジストリ ビューのみを読んでいましたが、Evergreen ランタイムは 32 ビット ビュー（`WOW6432Node`）に登録されるため、64 ビット ビルドは常に未インストールと判断し、誤った「WebView2 をダウンロード」案内を表示し、キャンセルするとアプリが終了していました。両ビュー × 両ハイブとディスク上のインストール先を確認するようになり、起動後に実際に失敗した場合のフォールバックも追加しました。
- **死んだトンネルでも接続時間が増え続ける**：接続時間は接続時点からの実時間で加算していたため、スリープやスクリーンセーバーでハンドシェイクが止まってもカウントが進み続け、実際には夜間に切断されたトンネルが「22 時間」と表示されました。相手が応答しない間は時間が凍結し、復帰後に続きから加算され、詳細画面にもその状態が表示されます。

### 🔧 変更

- **リリース成果物のファイル名にバージョンを付与**：インストーラー、ポータブル アーカイブ、`.deb` / `.dmg` にリリース バージョンを含めるようになりました（例：`wireguideplus-2.3.1-amd64-installer.exe`）。ダウンロードしただけでバージョンが判別でき、同一フォルダー内で複数バージョンが上書きされ合うこともなくなります。更新チェッカーは元々ファイル名のトークンで照合するため、旧バージョンもこれまで通り自動更新できます。

### 🛠 内部

- **アイドル セッション監視をクロスプラットフォームの共通接続点に**：`SessionDetector` が Windows で復帰（画面オン / ロック解除）を通知し、他プラットフォームではノーオペレーションとして既存の再接続モニターに接続します。切断済みまたはハンドシェイクが古いトンネルのみ再接続するため、健全なトンネルは乱されません。
...
- **編集の保存が書き込み前に黙って中断されることがあった**：保存チェーンが一度もインポートされていないヘルパー関数を参照しており（`sanitizeTunnelName is not defined`）、そのエラーは UI に一切届きませんでした——通知もログもなく、ボタンを押しても何も起きない状態です。欠落していたインポートを修正し、保存チェーン上の予期しないエラーはすべて捕捉して表示するようにしました。
- **保存・インポートの失敗が英語のまま日志に残らない**：設定検証メッセージは生の Go 文字列でしたが、エラーコード方式に改め、5 言語すべてでローカライズされました。インポート（zip / QR / ファイル）・エクスポート・設定読み込みの失敗分岐もすべてログに書き込まれ、拒否された設定を追跡できるようになりました。
- **固まった保存は永久ハングではなく目に見えて失敗する**：編集保存チェーンの各ステップ（検証 → 切断 → 書き込み → 再接続）に 20 秒のウォッチドッグを設け、タイムアウト時はローカライズされたメッセージを表示します。ボタンが凍りつくことはもうありません。
- **ステータスプローブの失敗が保存を妨げなくなった**：保存中の接続状態プローブが到達不能だった場合、これまでは保存全体が生の英語エラーで中断されていました。
- **保存のたびに 4 件の偽「rpc timeout」ログ**：20 秒ウォッチドッグのタイマーは呼び出し成功後もキャンセルされず、時間になると成立しないタイムアウトとして発火していました。呼び出しが着地した時点でタイマーを解除するようにしました。

### 🛠 内部

- **保存チェーンの全ステップがログにパンくずを残す**：フロントエンド側のステップは新しい `LogFrontend` バインディング経由でログに書き込み、webview の未処理エラーと Promise リジェクションも同じログに捕捉され、設定更新のエントリポイントは呼び出しごとに記録します——サイレント保存は最後のパンくずから断たれた箇所を特定できます。

## [2.2.8] - 2026-09-19

### 🐛 修正

- **接続中にトンネルを編集しても保存されない（Windows / macOS 共通）**：バックエンドが接続中トンネルへの設定書き込みを拒否し、その失敗はログも通知もなく握り潰されていたため、「保存できたように見えて実際は旧設定のまま」でした。接続中でも編集可能にし、保存時はトンネルを切断して変更を書き込み、自動で再接続します。結果は「保存して再接続」「保存したが再接続に失敗」「保存に失敗し旧設定で再起動」の 3 通りを明示し、黙って失敗することはなくなりました。
- **遅延プローブのデータがトンネル間で混ざる**：トンネルが 1 本だけオンラインのとき、別のトンネルを選ぶとメイン トンネルの遅延と解決 IP が表示されました（helper は 2 本以上オンラインのときだけトンネル別統計を埋め、詳細ページは選択中トンネルが見つからないと全体ステータスにフォールバックしていました）。プローブ行は選択中トンネルに厳密に一致させ、一致しなければ何も表示しません。表示するのは実際に解決した IP のみで、他トンネル由来の元のターゲットをそのまま返すことはありません。
- **上流が切れても「接続中」のままハンドシェイクも正常に見える**：Wi-Fi や NIC は生きていてもルーターが上流を失うと、トンネルは「接続中」を報告しハンドシェイク時刻も正常でした。ステータスに「ハンドシェイク陳腐化」フラグを追加し、接続からしきい値を超え、かつその期間内に新しいハンドシェイクがない場合にのみ立てます（再接続直後を誤判定しません）。hero のドットとハンドシェイク行が琥珀色になり、一覧のドットも連動します。
- **狭いウィンドウでプローブ 2 列がずれる**：左の「プリセット」列見出しが右列の入力欄を 1 行押し下げていました。両列の見出しを削除し、左右が揃います。

### 🛠 内部

- **リリース ノートは再び手書きの CHANGELOG から生成**：本文は英語既定の `CHANGELOG.md` の最新 `## [x.y.z]` セクションを取り、`build/release-tail.md.tmpl` のダウンロード／チェックサム／Full Changelog／SignPath フッターを追加します。v2.2.7 以前の全リリースと同じページ構成に戻りました。git-cliff（および `cliff.toml`）を削除したため、本文はコミット メッセージではなく CHANGELOG に従います。

### 📝 ドキュメント

- **CHANGELOG を英語既定に**：`CHANGELOG.md` は英語（`README.md` と同じ命名規則）になり、簡体中国語版は `CHANGELOG.zh.md` に移動しました。

## [2.2.7] - 2026-09-18

### 🐛 修正

- **ドメインのエンドポイントで hero に現在の接続 IP が表示されない**：バックエンドが UAPI で解決済みのアドレスをプローブ候補として渡していたため、解決 IP が常に空になり hero にはドメインしか表示されませんでした。今は UAPI の実際のハンドシェイク アドレス（`status.endpoint`）を使い、ドメインのエンドポイントが接続されると hero に `host:port (ip:port)` を即時表示します（端点がリテラル IP の場合は重複表示しません）。
- **フィールド編集（Fields）で保存しても何も起きない**：保存ボタンが非同期の `SetTunnelFields` が終わる前に古い内容を dispatch し、入力を上書きするリアクティブが `load()` を繰り返し発火させていたため「押しても反応がない」ように見えました。`doSave` は `apply()` の成功を待ってから dispatch し、ボタンにビジー状態と「保存中」ラベルを付けました。

### 🔧 変更

- **遅延測定先エディタを 2 列に**：全トンネル（`0.0.0.0/0`）では左の「プリセット」（組み込み 8.8.8.8 / 223.5.5.5）と右の「カスタム」が各 2 行、分割トンネルは右のカスタム列だけを幅一杯で表示します。カード上部に AllowedIPs の参照行を追加し、測定先を編集しながら「どのアドレスがこのトンネルを通るか」をすぐ確認できます。遅延カードの行間を少し広げて余白を緩やかにしました。
- **詳細ページからルーティング无关の情報カードを削除**：詳細ページにあってはならない allowed_ips / 公開鍵 / DNS のカード（hero / ルーティング无关）を削除し、対応する死んだ CSS も削除しました。
- **狭いときは 1 列に戻る**：ウィンドウやページ全体を縮小して探測カードが 2 列を収めきれなくなったとき、探測先エディタは自動で 1 列に戻ります（カード自身の幅を見るコンテナ クエリなので、ページ全体の縮小でも正しく発動します）。入力欄が省略記号に潰れることはありません。

## [2.2.6] - 2026-09-18

### ✨ 追加

- **遅延測定先を保存したら即座に再測定**:これまで対象を変更しても次の 30 秒周期まで結果が出ず、「押しても何も起きない」ように見えました。IPC `Tunnel.ProbeNow` を追加し、保存成功後に測定サイクルを非同期で即実行します(RPC は即座に返り、15 秒の ICMP タイムアウトで保存操作を止めません)。結果は 1Hz の状態配信で反映されます。周期 tick と手動実行が重なった場合は後発を破棄し、同じ対象を二重に ping しません。結果が届くまで該当行に「測定中…」を表示します。
- **Linux / macOS でも pinned 出口インターフェースの消失を通知**:「出口インターフェースを明示的に pin → 自動フェイルオーバーしない」は、これまで実行時の GUI 通知が Windows のみで、Linux / macOS にはありませんでした。3 平台で同じ挙動になります:Linux / macOS は 2 秒ごとに pin されたインターフェースが存在し up かを確認し、消失時は同じ `EventEgressInterfaceLost` 通知でダイアログを出します(トンネルは pin を維持し、勝手に切り替わりません)。Windows 側の監視も名前で解決する方式(`net.InterfaceByName`)に変更し、名前変更後も追跡できます。

### 🐛 修正

- **トンネルの新規作成 / 編集で保存しても何も起きない**:フィールドエディタ(Fields タブ)が `errors` を宣言も描画もしておらず、親側も渡していなかったため、検証エラーも保存失敗も完全に黙って握りつぶされていました。設定エディタと同じエラー表示を編集領域上部に出すようにしました。
- **分割トンネルで遅延測定先が誤って「AllowedIPs の範囲外」と判定される**:Wails の IPC はフィールドを snake_case(`peers` / `allowed_ips` / `public_key` / `interface.dns`)でシリアライズするのに、フロントエンドは camelCase で読んでいたため値が取れず、カバレッジ検証が「有効な範囲が 1 つもない」と判断してあらゆる IP / ホスト名を拒否していました。併せて、詳細ページの公開鍵と DNS が空表示になる問題も修正しました。
- **エンドポイントなどを選択してコピーできない**:hero のエンドポイントを「クリックでコピーするボタン」から選択可能なテキストに戻しました —— ユーザーが欲しいのは大抵その一部(ホスト名だけ、または括弧内の実 IP だけ)であり、ボタンでは常に全体しか得られません。選択時の反転表示も欠けていました:アプリは全体で選択を無効化しており、opt-in していた少数の領域は UA 既定色(暗いカード上ではほぼ見えない)のままでした。1 つのグローバル規則でテーマのアクセント色に統一し、入力欄 / テキストエリアも明示的に選択可能に戻しました。

### 🔧 変更

- **トンネル詳細ページを整理**:hero と重複していた独立のエンドポイント表示ブロックを削除。遅延カードと遅延測定先の入力欄を同じ行に配置。入力欄下の長い説明は「詳細」トグルに折りたたみ、常時は 1 行のみ表示。「ハンドシェイク / 経過時間 / インターフェース名」は hero に移動してサイズを拡大。ドメインのエンドポイントには現在の解決アドレスを併記(`host:port (ip:port)`)します。値はフロントエンド独自の DNS 引きではなく測定ストリームから取得し、エンドポイントが既にリテラル IP の場合は重複表示しません。

## [2.2.5] - 2026-09-17

### ✨ 追加

- **macOS / Linux のシステム状態バブルを Windows と統一**：これまで Windows のみがシステムトレイバブルを表示し、macOS / Linux の `showStatusPopup` は空実装でした。クロスプラットフォームのバブルロジックを抽出（`popup_wails.go` + `popup_state.go`）し、macOS / Linux も同じバブルを表示するようになりました——起動 10 秒後（権限昇格プロンプトが落ち着いた後）に現在の接続状態を表示し、ネットワーク変化でトンネル状態が変わった後は安定した最新状態を遅延表示します。バブルには操作メニュー（メインウィンドウを開く / 切断）、手動閉じ、設定した駐在時間（既定 10 秒）での自動閉じがあります。
- **遅延プローブの複数ターゲット並行**：全トンネルは 8.8.8.8 + 223.5.5.5 + エンドポイント + 手動値（重複排除）を同時にプローブし、「最初に応答したもので終了」はしなくなりました。分割トンネルはエンドポイント + 手動値のみをプローブします。各候補は専用の行で表示され（状態ドット + 名前 + 解決 IP + 種別タグ + ミリ秒、≤100ms 緑 / ≤300ms 黄 / それ以外は赤または不通）。
- **遅延プローブ対象の AllowedIPs カバレッジ検証**：分割トンネル（AllowedIPs に 0.0.0.0/0 なし）でカバレッジ外の手動 IP は保存時に拒否され赤文字で表示されます。全トンネルは検証しません。ドメインは保存時に解決され、解決できなければ拒否されます。4 スロットで対象を保存（2 つは公共カバレッジ + 2 つは任意型）し、カバレッジは毎サイクルの現在の設定から再計算されるため、保存後にのみ無効になる対象も検出されます。
- **終了時切断トグル**：設定に「終了時にトンネルを切断」の明示トグルを追加（既定オン、現状維持）。オフの場合は終了時に Shutdown を使用してトンネルを維持します。

### 🐛 修正

- **接続状態の誤表示**：これまで `StateConnecting` がアクティブトンネルとして数えられ、トレイ / アイコンが早く緑になり、履歴に「接続直後に切断」が残っていました。新たに「確立済みトンネル」（ハンドシェイク完了後のみ真の接続）を視覚に使用し、「接続中」の中間状態を追加しました。履歴はハンドシェイク成功後にのみセッションを記録し、ハンドシェイクしない試行は静かに破棄します。
- **起動時 DNS 未準備で接続失敗**：新たな `resolveEndpointWithRetry`（全体 15 秒 / 1 回 5 秒 / 0.8 秒倍増バックオフ）は、エンジン構築前に瞬間的な DNS 失敗のみを再試行し、起動時 DNS 未準備による接続失敗を回避します。
- **IPv6 MTU 警告ノイズ**：致命的でない IPv6 サブインターフェース欠落（「要素が見つかりません」）は Debug に降格、その他のエラーは Warn のままです。

### 🛠 内部

- **非 Windows ビルド修正**：`popup_state` の型と定数が `//go:build windows` ファイルで定義されており macOS / Linux のコンパイルが失敗していました。ビルドタグなしの `popup_state.go` に抽出しました。
- **`internal/diag` 自己衝突 live-repro テストの環境対応**：これまで `selfAddrs=10.30.35.2/32` をハードコードしており、実際のアダプタ住所と食い違って自己衝突を誤報していました。現在は実際のインターフェースから住所を走査し、該当形態でない場合はスキップします。
- **CI / ビルド**：アクションバージョンを釘付け（checkout v5.1.0 / setup-go v6.5.0 / setup-node v5.0.0 / upload-artifact v7.0.1 / git-cliff v4.9.0）、dependabot を追加。公開ハイジーン用 pre-commit フック + CI スキャン（コミットメッセージ／リリースファイルの敏感語）。一貫した改行のため `.gitattributes` を追加。CONTRIBUTING のバージョン行列を同期。

### 📝 ドキュメント

- **README**：複数ターゲット遅延プローブ、macOS / Linux 状態バブル、終了時切断を追記（5 言語）。

## [2.2.0] - 2026-09-16

### ✨ 追加

- **トンネル名の検証とサニタイズ（共有モジュール）**：新しいフロントエンドモジュール `tunnel-name.js` を追加し、バックエンドの `ValidateTunnelName()` のすべての規則（許可文字 `A-Za-z0-9-_ `、64 文字以内、先頭・末尾の空白禁止、Windows 予約デバイス名の禁止）をミラーしました。**インポート**、**設定エディタの名前欄**、**詳細ページのリネーム**の 3 か所で共有され、動作が完全に一致します。
- **名前の自動修正を通知**：名前に含まれる不正な文字（`.`、`()`、`·`、非 ASCII など）は `-` に自動置換され、トーストで「元の名前 → 保存された名前」を通知するため、書き込み失敗と誤解されることがなくなります。

### 🔧 変更

- **手動リネームの挙動をインポートと統一**：これまで手動リネームで不正な文字があるとバックエンドの英語エラーがそのまま表示される一方、インポートは無音で名前を修正しており、挙動が一致していませんでした。現在は**不正な文字を常に自動置換し、通知します**。サニタイズ後も成立しないのは 2 つのみで、明示的なエラーになります：すべてが不正な文字（結果が空）と Windows 予約デバイス名（`CON` / `NUL` / `COM1` など）です。
- **エディタの名前欄が実際に保存される名前を表示**：保存後の名前欄には実際に保存された名前が表示され、「A のつもりが B で保存された」という事態を防ぎます。
- **プロンプトの多言語化**：インポート成功時のトーストはこれまで英語のハードコード（`Imported "x"`）でしたが、i18n に統一し、ファイル名が修正された場合は専用のメッセージで理由を説明します（5 言語対応）。

### 🛠 内部

- **予約デバイス名の追加**：`reservedDeviceNames` に `CONIN$` / `CONOUT$` を追加（`$` はそもそも許可文字集合外のため防御的エントリ）。さらに `TestReservedDeviceNamesCoverage` を新設してマップ自体を固定し、エントリが誤って削除されても検知できるようにしました。
...
- **プロキシミラープリセットの更新** — 設定 → 更新 の GitHub 高速化ミラープリセットを `ghfast.top` / `gh-proxy.com` / `ghproxy.net` / `mirror.ghproxy.com` の 4 つに変更。既存の「カスタムミラー」および「ローカルプロキシ」入力欄はそのまま残ります。
- **トンネル詳細の接続 / 切断ボタンの配色** — 接続ボタンを緑（go）、切断ボタンを赤（stop）に変更。どちらも塗りつぶしで、色相のみで区別されるため接続 / 切断が一目で判別できます。
- **自動化条件 UI の再構成**：
  - 「このネットワークで」条件を **「ゲートウェイ MAC で」** に名称変更。ゲートウェイ（ルーター）MAC アドレスでネットワークを指紋認識することを明確にし、識別性を高めました。併せて「ゲートウェイ MAC 指紋」説明を追加。
  - **「任意の Wi-Fi」** 独立オプションを削除 —— Wi-Fi SSID 条件は具体的な SSID の入力が必須になり、「任意の Wi-Fi」ショートカットはなくなりました。
  - **「有線ネットワーク（Ethernet）」** 独立条件を削除 —— インターフェース（on interface）ドロップダウンは**すべてのネットワークアダプタ**（Wi-Fi カードを含む）を列挙するように。有線ネットワークの真の判別軸（ゲートウェイ IP / ゲートウェイ MAC / サブネット）はすでに別条件として存在します。
  - デフォルト状態の意味は変わらず：条件を何も指定しないルールはすべてのネットワークに適用されます（これが「任意」のフォールバックです）。

### 🛠 内部

- 古い `ethernet` 自動化ルールはバックエンドエンジンで引き続き互換（データ喪失なし）。新規ルールではこの条件タイプを作成できなくなりました。

## [2.0.0] - 2026-09-13

### ✨ 追加

- **手動接続 / 切断と上書きラッチ（Manual Override）**：メインの操作ボタンは明示的に手動動作となり、自動ルールを無視して強制接続 / 切断します。トンネル詳細に「制御元」チップを表示——自動（ルール / デフォルト兜底）、手動切断（ルール停止）、手動接続（ラッチ済み）——ホバーで全意味を説明。
- **デフォルト兜底の生效表示**：ルールに一致せずデフォルト状態に収束した場合、デフォルト状態行の外枠が一致ルールと同様に強調され、「生效中：デフォルト兜底を実行」と表示。
- **トンネル状態の用語整理**：「接続済み」（インターフェース起動 + ハンドシェイク完了、通信可）、「接続中」（インターフェース起動済みだがハンドシェイク未完了の半開状態）、「切断済み」（インターフェースなし）を区別し、詳細なツールチップを付与。
- **ルート可視化の強化**：オンリンク LAN ルート（ゲートウェイ `-`）のフィルタトグルを追加；セルラー（Cellular）インターフェース種別ラベル；「-」、ARP / ネイバーエントリの自動非表示、仮想 / サードパーティアダプタの作成アプリ名表示を説明するヘルプポップオーバー。
- **DNS 漏洩テストの堅牢性**：公開 DNS リスト元（public-dns.info）の取得に失敗しても組み込みの公開 DNS リストへ自動フォールバックし、テストに影響なし（明確なヒント付き）。
- **自動化「生效」判定の説明**：一致に必要な 4 条件（条件が現在のネットワークに合致、リスト内で最初の一致、エンジンが実際に動作を実行、トンネルの実状態が動作と一致）をツールチップで詳述。正しく実行された切断はトンネルが down になるのが「生效」であり、半開状態は生效を名乗らないことを明記。

### 🔧 変更

- **トンネル詳細のメイン操作を再設計**：接続ボタンにブランド深紅の左バー（#A01D21、アプリアイコンに呼応）を追加；CTA を拡大し影を強化；切断ボタンは薄い赤の枠線を常時表示；メイン操作は sticky で長いパネルでも視界から消えない；2 行サブタイトルはツールチップへ移動。
- **テーマとコントラスト**：ダークモードの `--text-muted` コントラストを AA（約 4.8:1）に向上；ダークモードの `--green` をアイコンの医用グリーン（#22c55e、AAA）に合わせ；冗長なトークン（`--blue-tint`、`--accent-blue`）を削除し `--accent-tint` に統一。
- **ドキュメント**：README は WireTunnels を推奨しなくなり；2.0「Windows システムサービス」ロードマップを削除（その目標は計画外に）。

### 🛠 内部

- **新規テスト**：手動上書きラッチ、インターフェース帰属、DNS 漏洩公開リスト取得；新しい意味とデフォルト兜底をカバーするよう自動化評価テストを更新。
- **i18n ファイルを 515 キーで統一**（zh / en / ja / ko / zh-TW）。

## [1.8.5] - 2026-09-12

### ✨ 追加

- **ポリシー層（Policy Layer）**：トンネルスナップショット上の純関数アナライザ——プレフィックス分類、ルート重要度、DNS と保護チェック。
- **競合ポリシー**：保存時は許可 + ログ；手動接続時はブロック + 競合警告；自動化発動時はブロック + 通知；AllowedIPs は書き換えない。
- **トンネルごとのポリシー**（`.meta` サイドカーに保存）：traffic_protect、domains、use_as_default_dns、dns_resolve_path、system_dns。新規 TunnelPolicies UI。
- **ルートデータ契約**：L3 次ホップのみのゲートウェイ（オンリンクは `—`、MAC と ARP / ネイバーは除外）、正規化された宛先、ユニキャストのみ。
- **診断の強化**：自動化ネットワーク板にインターフェース種別ラベル；より詳細な接続 / 切断ログ；コンパクトなトンネルエディタレイアウト。

### 🔧 変更

- **グローバル Kill Switch を削除**：設定 / IPC / CLI / helper から削除。関連保護はポリシー層がトンネルごとに担う。

## [1.8.0] - 2026-09-09

### ✨ 追加

- **実行ログを大幅に充実** — GUI は起動時にバージョンとプラットフォームを記録。トンネルの接続/切断リクエストと結果（失敗は原因付きで WARN 記録）、設定のインポート（zip 一括インポートの成功/失敗集計を含む）、保存、削除、リネームにもログを追加。自動化エンジンはトンネルごとに判断（接続・切断・スキップとその理由）を記録し、手動切断の「ラッチ」は設定・解除の両方で通知。「ルールがなぜ接続しないのか」の調査が当て推量ではなくなります。
- **ルートテーブルのゲートウェイに「オンリンク」表示** — `link#N` のリンク層ゲートウェイを、ローカライズされた「オンリンク」ラベルで表示（元の値はツールチップで確認可能）。

### 🔧 変更

- **自動化ポリシーを即時反映** — 自動化エディターでルールや既定状態を保存すると、次のネットワークイベントを待たずに直ちに再評価を実行します。
- **ログにカテゴリフィールドを追加** — ログエントリに `category`（tunnel / network / app）が付き、主題ごとの絞り込みが容易に。

### 🐛 修正

- **AllowedIPs 競合検出の誤検出を完全修正**（1.7.9 の不完全な修正の補完）:
  - トンネル自身のインターフェースを**アドレスで識別**（トンネル自身の Address を持つインターフェースはスキップ）し、ヘルパー状態の応答への依存を排除 — CLI の `connect` 事前チェックや GUI 状態取得失敗時でも、トンネル自身のルートを競合として数えません。
  - 新しい CIDR が既存のより広いルートの**内部に厳密に含まれる**場合は警告しない：最長プレフィックス一致ではより具体的なルートが常に優先され、結果は決定的で曖昧さがありません。従来は Clash/Mihomo TUN の準デフォルトルート（例：`8.0.0.0/5` が `10.0.0.0/8` 全体を覆う）により、すべてのプライベート範囲が「競合」として報告されていました。同一プレフィックス（2 つのトンネルが同じ範囲を主張）と新しいスーパーセット（フルトンネルが既存ルートを覆う）は引き続き報告します。
- **既定状態のみでルールのないトンネルが評価されていなかった** — エンジンは「ルールがある」トンネルだけを走査しており、1.7.9 の「既定状態単独で有効」は実際には機能していませんでした：既定接続のトンネルはルールなしでは自動接続しません。現在は既定状態とルールの和集合を評価します。
- **ルートテーブル表示の修正** — macOS の省略表記の宛先を完全な CIDR に展開（`2/7` → `2.0.0.0/7`）；宛先とゲートウェイ列の列幅を修正し、長いアドレスが隣の列に溢れたり切れたりしなくなりました（省略記号 + ホバーで完全な値を表示）。
- **フィールドエディターで Endpoint を必須表示** — Endpoint が他の必須フィールドと同様に赤い \* マークを持つようになりました。

### 🛠 内部

- 競合検出の回帰テスト（サブネット抑制 / 同一プレフィックス / 新スーパーセット / 実機検証）、macOS ルート展開テスト、自動化再評価トリガーのテストを追加；多言語ファイルを 456 キーで整列。

## [1.7.9] - 2026-09-08

### ✨ 新機能

- **トレイメニューにバージョン表示** — メニューの最初の行が「WireGuide Plus」+ 実行中のバージョンになり、稼働中のビルドがひと目で分かります。
- **フィールドエディターの必須マーク** — PrivateKey、Address、PublicKey、AllowedIPs などの必須フィールドに赤い \* マークを付け、ホバーでローカライズされた説明を表示します。

### 🔧 変更

- **デフォルト状態のみで有効に** — ルールを 1 件も追加せずデフォルト状態だけを設定したトンネルが、**常にその状態に収束する**ようになりました（以前はこの構成は完全に無視されていました）。ルールもデフォルト状態も無いトンネルは従来どおり一切操作しません。これに伴い、`automation default` は既存ルールを必要とせず、最後のルールを削除してもデフォルト状態は保持されます。
- **エディターのレイアウト改善** — 設定編集ダイアログを拡大（860×620 → 900×760）。スクリプトエディターの 4 つのフックを 2 列配置に（PreUp+PostUp を 1 行、PreDown+PostDown を 1 行）し、パネルをコンパクトに。フィールドエディターのラベル列を広げ、PersistentKeepalive が入力欄と重ならなくなりました。

### 🐛 修正

- **macOS の自動起動が機能しない問題を修正** — launchd が LaunchAgent 環境に `HOME` を提供しないため、起動直後にデータディレクトリを特定できずプロセスが即終了していました（plist は発火していたが即死）。パス解決がシステムアカウント情報（getpwuid）へフォールバックするようになり、LaunchAgent は `HOME` を明示的に注入し、Aqua セッションのインタラクティブプロセスとして宣言します。無効化時は登録解除してから削除します。
- **デフォルト接続のトンネルがログイン後に自動接続しない問題** — 起動時に必ず 1 回自動化評価を投稿するようにしました。ただしネットワークが未特定（SSID も物理インターフェースのアドレスも無い）の間は何もせず、SSID 報告やルートイベント到着後に実際の条件で判定を完了します。自動接続は引き続き機能し、すべての判定が条件評価に基づきます。
- **AllowedIPs 衝突検出の誤検知を修正** — 接続中のトンネルが**自分自身のインターフェース**を衝突源として数えなくなりました（以前は毎回全 CIDR が自己重複として報告されていました）。重複判定は厳密な交差判定に引き締め、兄弟プレフィックス（例：`10.30.30.0/24` と `10.30.35.0/24`）は包含衝突と誤認されなくなり、アドレスファミリ（IPv4 vs IPv6）をまたぐ比較も行いません。

### 🛠 内部

- 削除済みの「otherwise」条件とデフォルト状態のセマンティクスに合わせてテストスクリプトとドキュメントを更新。CIDR 重複の回帰テストとエンジンレベルのデフォルトのみポリシーの単体テストを追加。
