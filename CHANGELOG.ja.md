...
- **自動化ルールを「デフォルト状態 + 順序付きルール」に変更** — 各トンネルは優先度順のルールリストを持ちます：上から順に評価し、**最初に一致したルールが有効**（ルール内の複数条件は AND）。どれも一致しない場合はそのトンネルの**デフォルト状態**（接続 / 切断）に収束し、「otherwise」分岐は廃止されました。デフォルト状態は自動化エディター上部で設定し、ルールはドラッグで並べ替え、アクションバッジをクリックするだけで接続 / 切断を切り替えられます。
- **望ましい状態への収束（Desired State → Reconcile）** — エンジンは各トンネルを「あるべき状態」へ継続的に近づけます：「接続」に一致すれば接続、「切断」に一致すれば切断。トンネルがどのように確立されたかは関係ありません。ルールが 1 つも無いトンネルは従来どおり一切操作しません。
- **CLI に `automation move` と `automation default` を追加** — `wireguideplus ctl automation move <トンネル> <元番号> <新番号>` でルールの優先度を並べ替え、`wireguideplus ctl automation default <トンネル> <connect|disconnect>` でデフォルト状態を設定します。

### 🐛 修正

- **ネットワークイベントの連発で自動化が繰り返し発火しないように** — Wi-Fi 接続時に発生する複数の SSID / ルート変更イベントは 1 回の評価にまとめられます：トリガーは 1 スロットのメールボックスに投げるだけで、評価開始時に常に最新のネットワークコンテキストを再取得し、評価中に届いたイベントは最大 1 件だけ次の評価として残します。滞留も、古い状態での判断も起こりません。
- **終了時の自動化後処理の競合** — シャットダウン開始後に保留されていた評価リクエストは実行されなくなり、「全トンネル切断」のクリーンアップと同じトンネルを取り合うことがなくなりました。
- **旧設定の「otherwise」ルールを自動移行** — 旧設定を読み込む際、最初に有効な `none_match` ルールをそのトンネルのデフォルト状態に変換し、それ以降（完全に覆い隠されていた）ルールを削除します。「otherwise」が無い設定はデフォルト「切断」に移行します。移行は冪等で、従来の挙動を保ちます。

### 🛠 内部

- **調整アクションを純粋関数に抽出し冪等性テストを追加** — 同じネットワークコンテキストで 2 回続けて評価しても 2 回目の状態変更アクションは発生しません。手動でオフにしたトンネルはどの経路でも自動再接続されません。
- **設定移行 / CLI / GUI の 3 つの書き込み経路の整合性テスト** — すべての経路が同じ正規化モデルに着地します：ディスクに `none_match` を残さない、ルールがあれば有効なデフォルト状態がある、ルールが無ければデフォルト状態も無い。
- **ドキュメント更新** — 設計書にデフォルト状態と順序付きルールの意味、イベント合併とロック順序を追記。手動テストチェックリストから廃止された「otherwise」ケースを削除し、イベント連発と終了タイミングの検証を追加。
...
- **トランスルートスクリプト（PreUp / PostUp / PreDown / PostDown）エディター** — トンネル編集ダイアログにスクリプトパネルを追加：既存スクリプトの選択（任意フォルダ）、空スクリプトの新規作成（既定で scripts フォルダに作成、名前変更可）、コードの編集、ワンクリックでクリア。フックはテキスト置換で書き戻すため、conf 内のコメントや手動の行順は保持されます。
- **独立した scripts フォルダと設定のエクスポート/インポート** — tunnels・logs と並ぶ scripts データフォルダを追加。設定に「設定のエクスポート」（tunnels + scripts + config.json をバンドル、ログは含まない）と「設定のインポート」を追加し、環境移行を簡単に。
- **フィールド編集ビュー** — トンネル編集ダイアログに conf / フィールドのタブを追加：フィールドタブは conf の各キーに対応する入力欄（interface / peer グループ）を表示し、AmneziaWG の難パラメータはスイッチが有効なときのみ表示。保存時は conf テキストへ書き戻し、コメントとカスタム行はそのまま保持。
- **トンネルごとの物理出口バインディング（Windows / Linux / macOS）** — Pin Interface 有効時、トンネルごとに暗号化トラフィックを特定の物理 NIC に固定可能：Windows はソケットレベルの IP_UNICAST_IF バインド、Linux は `ip route … dev <iface>` のバイパスルート、macOS は `-ifscope` 付きルート。バインドに失敗した場合はデフォルトルートへフォールバックせず接続を失敗させ、トラフィック漏えいを防止。
- **NIC消失時の処理ダイアログ** — バインドした出口 NIC が消えた場合、黙ってフォールバックせず「バインドを保持して復帰を待つ」「利用可能な NIC へ自動切替」「トンネル編集を開いて手動指定」の 3 つの選択肢を提示。消失イベントごとに一度だけ通知。
- **AmneziaWG スイッチの二段階確認と統一バッジ** — 設定で AmneziaWG サポートを有効化する際、Pre/Post Scripts と同じハイライト確認を要求。トンネル一覧と詳細のバッジは AmneziaWG ON（紫）/ AmneziaWG OFF（赤）に統一。

### 🐛 修正

- **NIC 一覧が一般名称を表示** — Windows のドロップダウンは以前ハードウェア記述（Realtek… など）を表示していましたが、システムの一般名称（イーサネット、WLAN など）を優先表示し、ハードウェア型番は補助情報に。Linux は Wi-Fi / Ethernet / Cellular に分類し、全 NIC がデフォルトルート保持と誤判定される問題を修正。macOS はシステムのネットワーク設定からポート名を取得。
- **フィールドエディターが開かない** — タブ切替が未定義の変数を参照してクリック時にエラーになる問題を修正。切替時に現在の conf テキストを再解析します。
- **初回起動時に AmneziaWG バッジが誤って ON 表示** — バッジは設定の読み込み前に既定値で描画されていました。読み込み済みの設定のみを反映し、状態が不明な間は非表示に。
- **Pin Interface が誤って「このプラットフォームでは非対応」と表示** — アクティブなトンネルがあると切替が非対応扱いでロールバックされていました。アクティブなトンネルは現在のバインドを保持し（再接続時に反映）、結果はログにのみ記録。
- **Linux の出口バインディングがデフォルトルートへフォールバックしない** — バイパスルートの設置に失敗した場合は接続を失敗させ、デフォルト出口へのトラフィック漏えいを防止。

### 🛠 内部

- **ダイアログのドラッグとリサイズに対応** — 設定 / Automation / トンネル編集ダイアログをタイトルバーで移動、角からリサイズ可能。編集ダイアログは既定サイズを拡大し、右上に閉じるボタンを追加。「conf テキスト」タブは「conf エディター」に改名し、フィールドタブのボタン様式を conf タブと統一。
- **フィールド編集のグループを全幅表示** — interface / AmneziaWG / peer グループすべてがエディター幅いっぱいに広がり、長い値が半分の幅に圧迫されなくなりました。
- **ローカルビルドのアイコンとバージョンリソースを補完** — ローカルビルドも CI と同じ手順でリソースファイルを生成し、テスト用実行ファイルにアイコンが欠けなくなりました。正式リリースは引き続き CI がビルドします。

## [1.7.1] - 2026-09-06

### ✨ 新機能

- **旧バージョンデータの通知を刷新** — 旧バージョン（"wireguide" フォルダ）が残したデータを検出した際、ダイアログで「旧フォルダ／新しい設定フォルダを開いて確認する」「新しい設定フォルダへ移動する」「今後表示しない」「今回はキャンセルして次回起動時に再度通知する」を選べるようになりました。
- **移動時の同名ファイル競合への対応** — 移動先の設定フォルダに同名ファイルが既にある場合、ダイアログに競合ファイルを一覧表示し、「上書きして移動」「新しいフォルダを開いて内容を確認」「今後表示しない」「今回はキャンセルして次回起動時に再度通知」から選択できます。

### 🐛 修正

- **移行をスキップすると旧データが削除されていた問題** — 以前は移行を呼び出すたびに無条件で旧フォルダを削除していたため、「後で通知」を選んだ場合や同名競合でスキップされた場合に、まだ移行していないデータが密かに失われ、以後二度と検出できなくなっていました。すべてのファイルが実際に移動され（スキップなし）た後にのみ旧フォルダを削除するよう変更しました。

### 🛠 内部

- **設定画面の移行項目を削除** — 旧バージョンファイルの検出は初回起動時の自動実行のみとなり、設定画面から手動で再検出する必要がなくなりました。該当する設定セクション、バックエンドのリセット処理、および状態保存をあわせて削除しました。
- **移行文言の全言語対応** — 新しい競合通知と操作ボタンを 5 言語すべてにローカライズしました。
...
- **自動化エディタ：草稿編集で即時再評価**。ルール追加・ドラッグ並び替え・条件タイプ切り替え・入力・曜日チェックなどの変更は、約 250 ms のデバウンスで直ちにバックグラウンド評価エンジンへ IPC で渡され、match / in use バッジと最上部の判定バーが 3 秒間隔のポーリングを待たずに更新されるようになりました。実際の helper の制御と同じエンジンを使うため、表示と動作は常に一致します。
- **自動化エディタのレイアウトを高密度化**。ルールカードのパディング、カード間隔、条件行のパディング、各入力コントロールと曜日ボタンを縮小。同じ表示高さに収まるルール数が約 20% 増加。

### 🐛 修正

- **詳細設定の 4 つのトグル + ログレベルで「IPC 成功後に保存」の共通フローを導入**。Kill Switch / DNS 保護 / Pin Interface / 健康診断 / ログレベルはこれまで「設定値を先に書き込み、失敗したらロールバック」という実装で、失敗時には settings.json に不正な値が残る、失敗が UI に通知されないという問題がありました。5 項目とも helper の live-apply IPC を先に呼び出し、成功した場合に限りメモリ上の settings を更新して永続化するよう統一。失敗時は制御が以前の保存値へ自動で戻り、失敗理由を toast で表示します。ログレベルの `<select>` も同じフローに従います。
- **Wg Scripts 確認ダイアログで「キャンセル時にディスクへ書き戻さない」不具合**。これまでスクリプト有効化のスイッチを入れた瞬間に `enable_wg_scripts = true` が永続化されており、セキュリティ確認でキャンセルしてもメモリ上だけ false に戻り、settings.json 上は true のままでした。次回起動時「ON に見えるのにスクリプトは効かない」状態の原因です。永続化はダイアログの Confirm / Cancel ボタンからのみ行うよう修正。
- **Pin Interface トグルのトラック部分をクリックしても反応しない**。`.toggle input` が `position: absolute` なのに `inset` が未指定だったため、0×0 のヒット領域がトラック/スライダーの表示枠外へドリフトしていました。外側の `<label for="…">` 文字列ではなくスライダー本体をクリックしたときに特に顕在化していた不具合で、末尾の設定カードで最も頻発していました。input を `.toggle` コンテナ全体に敷くよう修正。
- **Automation ライブマッチインジケータが開くたびに内容がブレる**。複数の非同期エントリーポイント（onMount、初回 load、3 秒ポーリング、クローズ時一時停止タイマ）にセッション境界がなく、エディタを再オープンしたときに古い in-flight レスポンスが現在のトンネルの表示を上書きしていました。統一リフレッシュエントリーポイントを 1 つにまとめ、`previewEpoch`（セッション ID）+ `AbortController` をべき等キーとして使用、`open && !previewTimer` のリアクティブガードでオープン毎にポーリングを自動再作成、close/destroy で epoch・abort・timer を三種同時破棄する構成に直しました。
- **DNS リークテスト public-dns.info がボディ途中でタイムアウトしやすかった**。HTTP クライアントの 10 秒タイムアウトが UI 側 ctx タイムアウトと同程度で、混雑回線では 4–8 MB の `nameservers.json` ダウンロード中にトランスポート層が切断し、`json.Decoder` が読みかけで「parse JSON: context deadline exceeded」と表示する事態が多発していました。クライアントタイムアウトを 30 s に緩和、`LimitReader` を 16 MB → 4 MB に絞り（信頼度上位数百件のサーバーで十分、残りは下位ノード）、UI の 10 s ctx は引き続き最優先でキャンセル可能な構成としました。

### 📝 ドキュメント

- **5 言語版 README に「自動化ルール」の独立セクションを追加**。ルールの評価セマンティクス（ルール内 AND、ルール間は OR で最初に一致したもの勝ち、disconnect グループ先行、otherwise フォールバック）、7 種類の条件の一致定義と実務的な使用例、ライブマッチインジケータ、CLI `automation` コマンドの説明を含みます。

## [1.6.0] - 2026-09-02

### ✨ 新機能

- **自動化ルールに 4 つの新しい条件を追加** — トンネルごとの connect/disconnect ルールで、ゲートウェイ IP（`gateway_ip`）、ネットワークインターフェース（`interface`、未接続の物理アダプタも候補に表示）、有線ネットワーク（`ethernet`）、時間帯（`time`、開始/終了時刻 + 曜日）を指定できるようになりました。ルール内の条件は AND、ルール間は OR で、保存順の最初に一致したルールが有効になります。disconnect グループを connect グループより先に評価し、後ろで一致した connect ルールは優先度を下げて実行されません。
- **自動化エディタのライブ判定** — 編集中、各条件の一致状況（match）を個別に表示し、現在実際に有効なルール（in use）をハイライトします。上部の判定バーに最終的に実行されるアクションを即座に表示します。判定は helper の実制御と同じエンジンを使用するため、表示と実際の動作は常に一致します。
- **CLI に `automation` コマンドを追加** — ターミナルから各ルールのライブ判定詳細と結果を確認できます。新しい条件タイプもすべて読みやすい形式で表示されます。

### 🐛 修正

- **SSID が完全一致（フルネーム）比較に変更**（動作変更）— SSID をバイト単位のフルネームで比較します：大文字小文字を区別し、途中の空白や特殊文字も一致判定に含まれます（802.11 における SSID の定義に準拠）。以前は大文字小文字を区別していませんでした。ルールに入力した SSID は実際にブロードキャストされている名前と完全に一致する必要があり、エディタのライブ判定で不一致が即座に表示されます。
- **「有線ネットワーク接続時」条件が保存されない** — ethernet 条件が永続化されず、保存後に黙って消え、ライブ判定のルール対応付けもずれていました。
- **エディタ表示インジケータの複数修正** — 未完成のドラフトルールがあると、後続のルールのハイライトがずれる問題を修正。手動オフ（manual-off）時は、抑制された connect ルールを「使用中」と表示しないように変更。エディタを開いたときに判定を即時更新し、最大 3 秒間古いデータが表示される問題を解消しました。
- **Windows のルート競合検出を修正** — 競合診断のルート競合は、`route print` の出力解析が信頼できなかったため Windows では常に空で、フルトンネル（0.0.0.0/0）のケースでのルート重複警告が機能していませんでした。iphlpapi の `GetIpForwardTable2` テーブルを使用するようになり、「診断 → ルート」ビューと一致します。

### 🛠 内部

- helper のルール評価ファイルを実際の役割に合わせて改名：`wifi_rules.go` → `automation_rules.go`、プラットフォームファイル `wifi_rules_{windows,darwin,linux}.go` → `userdir_{windows,darwin,linux}.go`。物理インターフェース列挙は `iface_*.go` に分割しました。

### 📝 ドキュメント

- README（5 言語）：macOS（Apple Silicon）のサポート状況を「実験的」から「完全サポート」に引き上げ。

## [1.5.3] - 2026-09-02

### 🐛 修正

- **保存したウィンドウ位置が実際に復元されるように** — 1.5.2 ではウィンドウのサイズと位置を保存していましたが、位置は再起動のたびに画面中央へ戻っていました。Wails のウィンドウ作成時に位置指定モードを明示していなかったため、各プラットフォームが既定で中央配置を実行し、保存した X/Y 座標を無視していたためです。有効な保存位置が存在する場合のみ絶対配置を適用するようになり、Windows / macOS / Linux すべてで再起動後に元の位置が復元されます。
- **Linux の位置復元オフセットを修正** — Linux ではウィンドウ表示後に保存座標を現在のモニター作業領域からの相対オフセットとして再適用していたため位置がずれていました。ウィンドウ表示後に絶対座標で一度だけ位置を再設定するようにしました。

### 🎨 UI 改善

- **ウィンドウ最小サイズを縮小** — 最小ウィンドウサイズを 920×640 から 720×560 に縮小し、詳細ペインがウィンドウに合わせて縮小（min-width: 0）できるようにしました。小さな画面 / 低解像度でもコンテンツに押し広げられることなくウィンドウを狭くできます。

## [1.5.2] - 2026-09-02

### ✨ 新機能

- **メインウィンドウの位置とサイズを記憶** — ウィンドウをトレイに閉じる、最小化する、またはアプリを終了するときに現在のウィンドウ形状を保存し、次回起動時（トレイからの復元を含む）に元どおり復元されます。

### 🛠 内部

- **GitHub Actions を最新メジャー版へアップグレード** — checkout@v5、setup-go@v6、setup-node@v5、upload/download-artifact@v7、action-gh-release@v3。

## [1.5.1] - 2026-09-02

### 🐛 修正

- **Windows の更新をサイレントインストールに戻しました** — 1.5.0 の対話型インストーラーを撤回し、ダウンロード完了後は UAC の承認 1 回だけで自動的に上書きインストールされ、完了後に新バージョンが起動します。
- **更新は常に元のインストール先を上書き** — 現在のインストールディレクトリをインストーラーに渡すため、インストール先をカスタムしたユーザーでも既定の場所（Program Files）に 2 つ目のコピーが作成されません。
- **「更新をサイレントでインストール」設定（1.5.0 で追加）を削除** — 更新動作は全プラットフォームで一貫しています：Windows はサイレントインストール＋自動起動、Linux はパッケージマネージャーによるインストール後に自動起動（システムの polkit プロンプトのみ）、macOS はアプリバンドルのインプレース置換（/Applications への書き込み時にシステムの認証プロンプト表示）。

## [1.5.0] - 2026-09-02

### ✨ 新機能

- **Windows の更新を対話型インストーラーに変更**（「今すぐ更新」が通常のインストールと同じウィザードを起動）。
- **「更新をサイレントでインストール」設定を追加。**

### 🐛 修正

- **Linux：更新完了後にアプリを自動起動** — deb/rpm パッケージマネージャーによる更新後、アプリが自動的に再起動され新しいバージョンが読み込まれます。

> 1.5.0 の対話型インストーラーとサイレント更新設定は、全プラットフォームでのサイレント更新を採用するため 1.5.1 で撤回されました。
...
- **パブリック DNS クロスチェック** — ローカル設定の DNS に加えて、既知のパブリックリゾルバ（Google、Cloudflare、OpenDNS、Quad9、Alibaba、DNSPod、114DNS、Baidu、AdGuard、NextDNS、Comodo、および一般的な IPv6 アドレス）にもプローブを送り、DNS クエリがトンネル内のみを経由しているかを突き合わせます。パブリックリゾルバの応答は「到達可能」を意味するだけで、リークではありません。
