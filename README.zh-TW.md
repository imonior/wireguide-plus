...
同一個開關也是隧道詳情頁頂部的一鍵式「停止自動化」開關，讓你在查看隧道時就能直接壓制自動化。全局「禁止系統休眠」設定（預設關閉）會在有隧道連線時讓電腦保持喚醒（Windows、macOS、Linux 三平台皆生效），最後一條隧道中斷後立即釋放，使閒置或休眠的電源策略不會悄悄殺掉隧道。當另一個用戶端已佔用某條隧道的位址時，WireGuide Plus 現在會彈出一個互動式對話方塊，指名衝突的軟體並主動提供「停止該隧道自動連線」——由你決定，它絕不強退另一個用戶端。

### 一條隧道，一個用戶端

兩個 WireGuard 用戶端不能同時執行同一條隧道：隧道的 `Address` 會先到先得地被某個介面卡佔住，後連接的用戶端會在指派地址一步失敗（Windows 報 `The object already exists.` / 物件已存在）。若你同時使用官方 WireGuard 用戶端，請先停止它的同名隧道服務（例如 `Stop-Service 'WireGuardTunnel$<隧道名>'`），或將該隧道排除出自動化（見上）。WireGuide Plus 會在啟動時偵測此衝突，在 helper 日誌中點名衝突介面卡，並對失敗的自動化連線以退避節奏重試（30 秒 → 1 分 → 2 分 → 5 分），而不是每個輪詢週期都硬撞介面卡。
