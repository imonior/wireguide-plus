...
- **螢幕熄滅、鎖定或睡眠時保持連線** — 一項設定（預設開啟，可依隧道覆寫），讓隧道在螢幕熄滅、螢幕保護與鎖定下保持存活，並在機器從睡眠喚醒後自動重連。
- **登入時自動啟動** — 登入後自動啟動 WireGuide Plus 並依規則連接（搭配「最小化啟動」
  視窗啟動即收攏）。
- **最小化啟動** — Windows 上啟動後最小化到工作列（工作列圖示保留，主視窗隨時可重新
  開啟）；macOS/Linux 上最小化到系統匣。
- **系統匣連線通知** — 啟動 15 秒後（管理員權限提示處理完成）彈出全部隧道總覽氣泡：逐條列出即時狀態與自動/手動模式，並按可設定的停留時間自動關閉（預設 10 秒，可於設定中調整）；
  之後網路變更（Wi-Fi 切換、拔除網路線、網路中斷等）導致隧道狀態改變時，彈出按隧道逐行顯示的狀態氣泡
  （「🟢 Connected: 名稱」綠 /「🔴 Disconnected: 名稱」紅 / 🟡 連線中琥珀），浮在其他應用程式之上但不搶焦點，
  **常駐直到你處理**：手動關閉、開啟主視窗、點斷開，或被更新的事件取代。氣泡停靠於螢幕工作區角落、不壓主視窗內容，按住可拖曳到任意位置。三平台行為一致（macOS/Linux 此前沒有彈窗）。
- **統一的系統匣操作** — 左鍵或右鍵單擊都開啟系統匣選單，左鍵雙擊開啟主視窗，三平台一致；選單項在 macOS/Linux 帶 🟢 / 🔴 狀態符號（Windows 選單由 GDI 繪製、無法渲染彩色 emoji，改用 ✔ / ✗）。
...
4. **系統整合** — 登入時自動啟動、啟動最小化（Windows 最小化至工作列 / macOS 與 Linux
   最小化至系統匣）、系統匣連線通知（啟動 15 秒後全部隧道總覽，停留時間可調整；狀態變更氣泡不搶焦點、
   常駐直到你處理）、統一的系統匣點擊（單擊開選單、左鍵雙擊開主視窗）。
5. **視窗標題與操作體驗最佳化** 等。
...
- **SSID 未知時失敗關閉**：若目前網路的 SSID 暫時未知——Wi-Fi 介面抖動後先恢復、身分回報尚未跟上（同一 SSID 下重新設定網路的情境）——以 SSID 為條件的規則按**無法判定**處理，該隧道本輪跳過。預設狀態絕不會悄悄壓過引擎尚無法評估的條件；SSID 重新已知的瞬間，評估隨即恢復。
...
手動覆蓋只持續到應用重啟為止。若某條隧道**永遠**不允許自動化驅動——典型情境是它同時被另一個 WireGuard 用戶端控制——請在隧道的自動化編輯器中把「自動化」開關關掉。該隧道會保留自己的規則與預設狀態，但自動化引擎既不會連接也不會斷開它。

同一個開關也是隧道詳情頁頂部的一鍵式「停止自動化」開關，讓你在查看隧道時就能直接壓制自動化。全局「禁止系統休眠」設定（預設關閉）會在有隧道連線時讓電腦保持喚醒（Windows、macOS、Linux 三平台皆生效），最後一條隧道中斷後立即釋放，使閒置或休眠的電源策略不會悄悄殺掉隧道。當另一個用戶端已佔用某條隧道的位址時，WireGuide Plus 會彈出一個常駐對話方塊，指名衝突的軟體、顯示該隧道的自動化狀態，並給出僅有的兩個選擇——停止本隧道自動化，或繼續嘗試——在你決定之前暫停該隧道的自動連線；未處理的衝突會在 GUI 啟動及 helper 重啟後再次彈出——由你決定，它絕不強退另一個用戶端。

### 一條隧道，一個用戶端

兩個 WireGuard 用戶端不能同時執行同一條隧道：隧道的 `Address` 會先到先得地被某個介面卡佔住，後連接的用戶端會在指派地址一步失敗（Windows 報 `The object already exists.` / 物件已存在）。若你同時使用官方 WireGuard 用戶端，請先停止它的同名隧道服務（例如 `Stop-Service 'WireGuardTunnel$<隧道名>'`），或將該隧道排除出自動化（見上）。WireGuide Plus 會偵測此衝突，在常駐衝突對話方塊與 helper 日誌中都點名衝突介面卡，並對失敗的自動化連線以退避節奏重試（30 秒 → 1 分 → 2 分 → 5 分），而不是每個輪詢週期都硬撞介面卡。
...
- Windows x64 安裝程式：`wireguideplus-<version>-amd64-installer.exe`
- Windows x86（32 位元）安裝程式：`wireguideplus-<version>-x86-installer.exe`
- Windows ARM64 安裝程式：`wireguideplus-<version>-arm64-installer.exe`

安裝程式檔名內嵌版本與架構（`wireguideplus-<version>-<arch>-installer.exe`，arch 為 `x86` / `amd64` /
...
- `WireGuidePlus-<version>-darwin-arm64.dmg` — 拖曳至「應用程式」的安裝器。
- `WireGuidePlus-<version>-darwin-arm64.zip` — 免安裝 `.app` 套件。
...
- **僅本地臨時簽章（ad-hoc），未經 Apple 公證。** 首次開啟時 macOS Gatekeeper 會攔截。可右鍵 →「開啟」，或執行一次以下指令清除隔離屬性：
  ...
  ```
- 同時提供 Homebrew cask（arm64 版）：`brew install --cask wireguideplus` 取得的也是同一個 `WireGuidePlus-<version>-darwin-arm64.zip`。
...
- `WireGuidePlus-<version>-linux-<arch>.deb` — Debian / Ubuntu 安裝套件。
- `WireGuidePlus-<version>-linux-<arch>-portable.tar.gz` — 免安裝二進位。

用套件管理員安裝 `.deb`，例如 `sudo apt install ./WireGuidePlus-<version>-linux-amd64.deb`；或解壓免安裝包後執行 `./wireguideplus`。免安裝版需要 GTK3 / WebKitGTK 執行期函式庫，`.deb` 會自動安裝；裸系統請先安裝：
...
日誌檢視器工具列與設定頁的日誌區都提供「打開日誌目錄」按鈕，可在系統檔案管理器中直接開啟該目錄。
