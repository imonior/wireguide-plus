...
手動覆蓋只持續到應用重啟為止。若某條隧道**永遠**不允許自動化驅動——典型情境是它同時被另一個 WireGuard 用戶端控制——請在隧道的策略面板中開啟「排除出自動化」。該隧道會保留自己的規則與預設狀態，但自動化引擎既不會連接也不會斷開它。

同一個排除開關現在也是隧道詳情頁頂部的一鍵式「停止自動化」開關，讓你在查看隧道時就能直接壓制自動化。新增的全局「禁止系統休眠」設定（預設關閉）會在應用程式執行時讓電腦保持喚醒，使閒置或休眠的電源策略不會悄悄殺掉隧道。當另一個用戶端已佔用某條隧道的位址時，WireGuide Plus 現在會彈出一個互動式對話方塊，指名衝突的軟體並主動提供「停止該隧道自動連線」——由你決定，它絕不強退另一個用戶端。
