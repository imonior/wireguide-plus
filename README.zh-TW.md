...
- **隧道編輯器：欄位檢視與腳本鉤子** — 除 conf 文字外，提供逐欄位表單（interface / peer 分組）編輯設定，並支援管理 PreUp / PostUp / PreDown / PostDown 腳本鉤子（選擇檔案、新建空白、編輯程式碼、清除）。
...
- **延遲探測** — 每隧道延遲探測支援多目標並行；每個候選（公共解析器 8.8.8.8 / 223.5.5.5、隧道端點，以及任意手填地址或網域）同時探測，並展示其解析 IP、類型與往返時延。分流隧道下手填目標會在儲存前按隧道 AllowedIPs 覆蓋校驗。
