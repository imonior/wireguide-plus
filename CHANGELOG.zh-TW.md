...
- **macOS 狀態氣泡改由 macOS 自身定位。** Wails 在 macOS 上從不回報主要顯示器，氣泡因此靜默退回「貼著主視窗」的兜底演算法——而那份座標混用了點與像素——最終落在螢幕中間；圓角卡片也被套在方形視窗裡，四角露出一圈硬邊框。現在尺寸、位置、圓角裁剪、置頂層級以及跟隨 Space／全螢幕，全部直接讀取 `NSScreen.visibleFrame` 由原生程式碼完成。
- **macOS 氣泡重新可以拖曳。** Wails 的 JS 視窗拖曳在 macOS 上從未生效，抓住氣泡毫無反應。現在改由原生滑鼠事件監聽器驅動：手勢依舊不搶焦點（既不會啟動應用程式，也不會奪取鍵盤焦點），而未達到移動閾值的點擊仍會原樣交給按鈕。
- **再次啟動一份程式不再引發「斷線／重連」彈窗風暴。** 兩個 GUI 實例共用同一個特權 helper，而退出的那一份會先拆掉隧道並關閉 helper——存活的那一份仍在依賴它。它的健康監視器據此判定 helper 當機，彈出管理員授權視窗重新安裝，自動化隨即重連隧道：於是主介面明明顯示已連線，彈窗卻在提示「已斷線」。GUI 現在在整個進程生命週期內持有單實例獨佔鎖；第二份啟動時僅請求已執行的實例顯示視窗，隨即退出，且發生在它觸碰任何共用狀態之前。

### 🔧 變更

- **macOS/Linux 狀態氣泡現在停在工作區右上角**，緊貼選單列下方——即 macOS 自己放通知的角落——而不是 2.4.0 引入的右下角，那處正壓著 Dock。Windows 仍依托盤所在角落停靠。
...
- **狀態氣泡支援滑鼠拖曳。** 常駐氣泡若剛好擋住你需要的內容，按住按鈕以外的任意區域即可拖走；整個過程不搶焦點、不啟動應用程式，三平台一致。Windows 氣泡走原生標題列拖曳（在氣泡內文單擊仍是開啟主視窗），macOS/Linux 氣泡走 Wails 視窗拖曳。

### 🔧 變更

- **macOS/Linux 狀態氣泡改為錨定螢幕工作區右下角** —— 與 Windows 氣泡停靠在工作列角落的位置一致，不再貼著主視窗右下角。通知不再壓在它所要回報的那個主視窗內容之上；僅在取不到顯示器資訊時才退回主視窗錨定。

## [2.3.11] - 2026-09-26

### ✨ 新增

- **日誌檢視器一鍵開啟日誌目錄** — 工具列新增「打開日誌目錄」按鈕，直接在 Finder / 檔案資源管理器 / 系統檔案管理器中開啟日誌所在資料夾（與設定頁日誌條目同源）。
- **系統匣選單狀態符號** — 已連線隧道顯示 🟢、未連線顯示 🔴（macOS/Linux；Windows 系統匣選單由 GDI 繪製、無法渲染彩色 emoji，改用單色 ✔ / ✗）。
- **三平台統一的系統匣點擊** — 單擊（左鍵或右鍵）開啟系統匣選單，左鍵雙擊開啟主視窗。

### 🔧 變更

- **狀態通知改為每隧道一行，三平台統一：**「🟢 Connected: 名稱」/「🔴 Disconnected: 名稱」/ 🟡 連線中，整行依狀態著色，取代原本的合併標題列。
- **狀態氣泡常駐直到處理** — 不再依計時自動消失：由你手動關閉、開啟主視窗、點斷開，或被更新的事件取代；先前離開電腦時的一條斷開警告可能在十秒後悄然蒸發。設定中的通知時長現在只作用於啟動總覽氣泡。
- **macOS 氣泡只浮出、不搶焦點** — 透過提升視窗層級浮在其他應用程式之上（前景是全螢幕應用程式時仍可見），不再啟動應用程式，通知出現在你正在做的事情*上面*，而不是*取代*它。

### 🐛 修復

- **網路瞬斷不再違背自動化策略盲重連** — 網卡瞬時掉線（例如改靜態 IP 會閃斷一下）時，斷線監視器過去會把快取的隧道無差別全部恢復，搶在每隧道自動化規則之前。現在兩條復原路徑都會先查詢同一策略：規則要求斷開的、網路未識別的、以及被暫停/豁免的隧道保持不動。

## [2.3.10] - 2026-09-24

### ✨ 新增

- **位址衝突會暫停該隧道的自動化，直到你做出決定。**當自動連線遇到已被其他用戶端佔用的位址時，現在會彈出常駐對話框：指明衝突來源軟體、顯示該隧道的自動化狀態，並給出僅有的兩個選擇——停止本隧道自動化，或繼續嘗試。沒有「稍後處理」的退路：在你回覆之前，helper 會掛起該隧道的自動連線；未處理的衝突會在每次 GUI 啟動以及 helper 重啟後再次彈出。
- **啟動總覽氣泡。**每次 GUI 啟動 15 秒後，會彈出一個列出全部隧道的氣泡：每條隧道的即時狀態與自動/手動模式，取代原先啟動後 10 秒的狀態氣泡。

### 🐛 修復

- **失敗的連線嘗試不再產生「已中斷」幻影氣泡。**只有真正連線過的隧道才會被播報為中斷；「連線中→失敗」不再被誤報為中斷，而真正的重新連線仍會播報。

### 🔧 變更

- **狀態氣泡在各平台採用統一語彙：**已連線＝綠色對鉤＋綠色文字，中斷＝紅色叉＋紅色文字，連線中＝黃色圓點——Windows 原生氣泡以矢量繪製，macOS/Linux 氣泡同樣如此，啟動總覽各列也一樣。

## [2.3.9] - 2026-09-24

### 🐛 修復

- **macOS/Linux：全新安裝後「無法開啟」已修復。** 特權 helper 以 root 執行，啟動時若使用者的日誌目錄不存在會代為建立——全新安裝（例如透過 .dmg 安裝 2.3.8）刪除了舊應用程式及其資料、但系統服務仍殘留時正是如此。該目錄此前被留成 root 擁有、0700 權限，導致之後每次 GUI 啟動都因「日誌目錄已存在但不可寫」而安靜結束。現在 helper 會將日誌目錄歸還給它所服務的桌面使用者，已受影響的機器在 helper 下次啟動時自動修復。若已安裝的 2.3.8 無法開啟，可立即執行 `sudo chown -R "$USER" ~/Library/Logs/wireguideplus`（Linux 為 `sudo chown -R "$USER" ~/.local/share/wireguideplus`）恢復，或直接安裝 2.3.9——首次啟動即自行修復。
...
- **Linux「禁止系統休眠」重新真正生效**：一次本地修改誤刪了 `systemd-inhibit` 呼叫，只剩一個不持有抑制鎖的空轉 shell，導致該設定在 Linux 上悄無聲息地失效。現已恢復 `systemd-inhibit` 對自守望 shell 的包裹，覆蓋鎖重新生效。
- **代理變更（用於更新檢查）現在即時廣播**：`proxy_mode` / `proxy_url` 從未進入 `settings_changed` 載體，導致設定介面的代理反射程式碼形同虛設，從其他用戶端或 CLI 修改代理也不會重新整理。現已將兩欄位加入載體並在變更時廣播。
- **設定儲存加固，杜絕欄位被重設**：儲存路徑改為先讀取最新設定、再僅覆蓋本畫面欄位（展開），新增加的設定項不會再被舊儲存路徑寫回零值。這從機制上消除了 2.3.6「無法儲存」的根因。
...
- **隧道詳情頁新增「停止自動化」雖關**: 一個醓目的單擊雙關，與設定頁的「排除出自動化」選項作用相同——開啟後引操不會再連線或中斷該隧道，直到你關閉它。
- **全域「禁止系統休眠」（保持背景執行）**: 新的可選設定，應用程式執行時讓電腦保持唤醒，使閉置/休眠的電源策略不會默默殺掉隧道。預設關閉——除非你主動開啟，否则不覆蓋系統電源策略。Windows 上呼叫 `SetThreadExecutionState`。
- **互動式位址衝突彈窗**: 當另一個 VPN 客戶端已佴用你的某個隧道位址時，應用程式會指名衝突的軟體和介面卡，並提供「停止該隧道自動連線」的選項。由你決定——它絕不會強制停止另一個客戶端。

### 🐛 修復

- **狀態列重複播報已連線的隧道**: 連線提示現在只上報狀態真正發生變化的隧道（已連線/已中斷），因此持續在線的隧道只會被播報一次，而不會在每個 30 秒狀態輯訊時反複彈出「X 已連線」。這正是「TS453Dmini 已連線」反複彈出的根本原因——實際上在抖動的是另一條隧道，而彈窗只是把當時仍上線那條的名字報了出來。
- **手動「停止自動化」現在覆蓋所有自動重連**: 失效連線的監控器现在同樣遵循每隧道的排除設定，因此關閉某隧道的自動化會真正阻止它被自動重連（此前監控器仍可能在握手超時時重連它）。

### 🔧 譪更

- **移除了舊的基於簽名的提示去重**: 它只是把氣泡藏起來，掩盖了真實的連線變化。现在改為在源頭通過上報真正的每隧道狀態變化來減少干擾。

## [2.3.2] - 2026-09-21

### ✨ 新增

- **將隧道排除出自動化**：隧道策略面板新增「排除出自動化」開關，開啟後引擎永遠不會連接或斷開該隧道，但保留其規則與預設狀態。適用於同一條隧道同時被另一個 WireGuard 用戶端控制的情境——手動覆蓋只能維持到下次重啟，而這個開關會一直生效。

### 🐛 修復

- **自動化每 30 秒硬撞隧道並狂刷狀態列**：當連線失敗——最常見原因是另一個 WireGuard 用戶端已經佔用了該隧道的 `Address`（Windows 報 `The object already exists`）——協調器在每個輪詢週期都會無退避地重試，反覆拆建 Wintun 介面卡，並每輪重新彈出連線氣泡。現在失敗的自動化連線會以退避節奏重試（30 秒 → 1 分 → 2 分 → 5 分，封頂），並在隧道轉為活躍、設定了手動閂鎖或網路規則變更時解除限流。
- **同一狀態反覆彈出「已連線」氣泡**：托盤現在按簽章對狀態通知去重——已經播報過（或剛被抑制）的狀態不再重複顯示，且首次抑制會作為 Info 記入日誌，而不是再次彈窗。
- **連線失敗在指派地址一步報得莫名其妙**：`AssignAddress` 現在把「地址已在本介面卡上」視為成功（冪等）；當地址確實被佔用時，會在錯誤中點名衝突介面卡，讓你清楚是哪一個別的用戶端在跑同一條隧道。
- **啟動時會警告同一條隧道在別處執行**：啟動瞬間 helper 會拿每條隧道的 `Address` 與機器真實介面逐一比對（排除自身介面卡），若發現被別的用戶端佔用，則在日誌中點名衝突介面卡並給出停止服務的提示。

### 📝 文件

- README 新增「一條隧道，一個用戶端」規則說明與按隧道「排除出自動化」開關介紹。
## [2.3.1] - 2026-09-21

### ✨ 新增

- **螢幕熄滅、鎖定或睡眠時保持連線**：新增「閒置時保持連線」設定（預設開啟，可依隧道覆寫），隧道連線時強制啟用持久 keepalive，使其在螢幕熄滅、螢幕保護與鎖定下不再悄然斷線。系統睡眠後喚醒會自動重新探測並重連。Windows 現在除系統睡眠外還監聽顯示器電源與工作階段鎖定/解鎖，螢幕保護或鎖定不再留下已斷的隧道。

### 🐛 修復

- **在已安裝 WebView2 的機器上誤報「未安裝」（Windows）**：啟動檢查只讀了 64 位元登錄檢視，而 Evergreen 執行階段實際上註冊於 32 位元檢視（`WOW6432Node`），因此 64 位元程式總是判定為缺失、彈出錯誤的「下載 WebView2」提示，按下取消還會直接結束程式。現在會探測兩個登錄檢視 × 兩個根機碼，並加上磁碟安裝目錄後備；另加一層「啟動後真失敗」的後備。
- **隧道已斷卻顯示不斷增加的連線時長**：連線時長依牆鐘時間累加，機器睡眠或螢幕保護令握手停止後計時仍持續上升——實際半夜已斷的隧道會顯示「22 小時」。現在對端無回應期間時長凍結、恢復後續算，詳細頁也會標註該凍結狀態。

### 🔧 變更

- **發佈產物檔名帶上版本號**：安裝程式、可攜壓縮檔以及 `.deb` / `.dmg` 都內嵌發佈版本（例如 `wireguideplus-2.3.1-amd64-installer.exe`），下載檔案即可辨識版本，同一目錄下多版本不再互相覆寫。更新器本就依檔名 token 比對，因此舊版本仍能正常自動更新。

### 🛠 內部

- **閒置工作階段監聽成為跨平台統一接縫**：`SessionDetector` 在 Windows 上回報喚醒（亮屏 / 解鎖）事件，其他平台為空實作，接入既有重連監控——且只對已中斷或握手逾時的隧道重連，健康隧道不會被擾動。
