...
- **編輯儲存可能在寫入前靜默中斷**：儲存鏈引用了一個從未匯入的輔助函式（`sanitizeTunnelName is not defined`），報錯完全不達介面——不彈提示、不記日誌，按鈕點了沒反應。已修復缺失的匯入，儲存鏈上的任何意外錯誤都會被捕獲並明確提示，不再無聲消失。
- **儲存／匯入失敗提示是英文且日誌無痕跡**：設定校驗訊息此前是裸 Go 字串，現已改為錯誤碼驅動並在五種語言中本地化。匯入（zip / QR 碼 / 檔案）、匯出與設定讀取的失敗分支都會寫日誌，被拒絕的設定從此可追溯。
- **儲存卡死會可見地失敗，而非永久懸置**：編輯儲存鏈的每一步（校驗 → 中斷 → 寫入 → 重連）都有 20 秒看門狗兜底，逾時給出本地化提示，按鈕不再僵死。
- **狀態探測失敗不再阻擋儲存**：儲存期間的連線狀態探測不可達時，此前會讓整個儲存以英文報錯收場。
- **每次儲存後出現 4 條假的「rpc timeout」日誌**：20 秒看門狗計時器在呼叫成功後沒有取消，到點照常觸發。現在呼叫一結束計時器即被清除。

### 🛠 內部

- **儲存鏈每一步都留麵包屑日誌**：前端步驟透過新增的 `LogFrontend` 綁定寫入日誌，webview 的未處理錯誤與 Promise 拒絕也被捕獲進同一份日誌，設定更新入口會記錄每次呼叫——靜默儲存從此可以從最後一條麵包屑定位斷點。

## [2.2.8] - 2026-09-19

### 🐛 修復

- **連線狀態下編輯隧道儲存無反應（Windows 與 macOS 皆存在）**：後端會拒絕向已連線隧道寫入設定，且失敗被靜默吞掉——既不記日誌也不彈提示，於是「看起來儲存成功、實際仍是舊設定」。現在連線中也可編輯：儲存新設定時會先中斷隧道、寫入變更、再自動重連。三種結果都會明確提示——「已儲存並重新連線」／「已儲存但重連失敗」／「儲存失敗，已用舊設定重啟」，不再靜默失敗。
- **延遲探測資料在隧道間串台**：只有一個隧道在線時，選中另一條隧道會顯示主隧道的延遲與解析 IP（helper 僅在多於一個隧道在線時才填充各隧道統計，詳情頁在匹配不到選中隧道時回退到全域狀態）。現在探測列嚴格依選中隧道匹配，匹配不到就一列都不顯示；只呈現真正解析出的 IP，絕不回顯來自其它隧道的原始目標。
- **上游已斷仍顯示「已連線」且握手正常**：Wi-Fi 或實體網卡還在，但路由器丟了上行時，隧道仍報「已連線」、握手時間也正常。現在狀態新增「握手陳舊」標記——僅當已連線超過門檻、且該時間窗內沒有新握手時才置位，剛重連的隧道不會被誤標；hero 圓點與握手列轉琥珀色，列表圓點同步。
- **窄視窗下探測兩欄錯行**：左側「預設」欄標題把右欄輸入框頂下一列。已移除兩欄標題，左右列對齊。

### 🛠 內部

- **發佈說明重新由人工維護的 CHANGELOG 產生**：發佈內文取英文預設的 `CHANGELOG.md` 中最新的 `## [x.y.z]` 段落，再附加來自 `build/release-tail.md.tmpl` 的下載／校驗／Full Changelog／SignPath 頁尾——還原 v2.2.7 之前每個版本一貫的頁面結構。已移除 git-cliff（及 `cliff.toml`），發佈內文隨 CHANGELOG 而非提交訊息產生。

### 📝 文件

- **CHANGELOG 改為英文預設**：`CHANGELOG.md` 現為英文（與 `README.md` 命名慣例一致），簡體中文版移至 `CHANGELOG.zh.md`。

## [2.2.7] - 2026-09-18

### 🐛 修復

- **網域端點 hero 不顯示目前連線的即時 IP**：後端把 UAPI 已解析位址當成探測候選傳給探測流程，導致解析 IP 始終為空、hero 永遠只顯示網域。現改為取 UAPI 實際握手位址（`status.endpoint`），網域端點連上後在 hero 即時顯示 `host:port (ip:port)`，字面 IP 不重複顯示。
- **欄位編輯器（Fields）點儲存毫無反應**：儲存按鈕在非同步 `SetTunnelFields` 完成前就派發了舊內容，且一個會覆蓋輸入的響應式反覆觸發 `load()`，看起來像「點了沒反應」。現 `doSave` 等待 `apply()` 成功後才派發儲存事件，按鈕加忙碌態與「儲存中」文案。

### 🔧 變更

- **延遲探測目標編輯器改為雙列**：全通道（`0.0.0.0/0`）時左側「預設」（內建 8.8.8.8 / 223.5.5.5）與右側「自訂」各兩列；分流通道只顯示右側自訂列、單列滿寬。卡片頂端新增 AllowedIPs 參考列，編輯探測目標時可直接對照「哪些位址才經此隧道」。延遲顯示卡文字行距略增，呼吸感更鬆。
- **詳情頁移除與路由無關的資訊卡**：刪除詳情頁不該出現的 allowed_ips / 公鑰 / DNS 卡片（與 hero / 路由無關），並清理對應的死 CSS。
- **自適應佈局回落單列**：視窗被縮放或整頁縮放導致探測卡過窄時，探測目標的雙列自動回落為單列（用容器查詢監聽卡片自身寬度，連整頁縮放也能正確觸發），輸入框不再被擠壓成省略號。

## [2.2.6] - 2026-09-18

### ✨ 新增

- **儲存延遲探測目標後立即重新探測**：以往改完目標要等下一個 30 秒週期才會出結果,看起來像「點了沒反應」。新增 IPC `Tunnel.ProbeNow`,寫入成功後立即非同步跑一輪探測(RPC 立刻返回,不被 15 秒 ICMP 逾時拖住),結果經 1Hz 狀態廣播推送;週期 tick 與手動觸發重疊時,後發的那次會被丟棄,不會把每個目標 ping 兩次;結果回來前該列會顯示「探測中…」。
- **Linux / macOS 回報 pinned 出口網卡遺失**:「明確 pin 出口網卡 → 不做自動故障切換」以往只有 Windows 會在執行時通知 GUI,Linux / macOS 缺位。現在三個平台一致:Linux / macOS 每 2 秒檢查被 pin 的網卡是否仍存在且為 up,遺失時沿用同一條 `EventEgressInterfaceLost` 通知鏈彈出提示(隧道維持 pin 狀態,不會悄悄切走)。Windows 側監控改為依網卡名稱解析(`net.InterfaceByName`),網卡改名後仍能持續追蹤,與另兩個平台簽名一致。

### 🐛 修復

- **隧道新增 / 編輯按下儲存毫無反應**:欄位編輯器(Fields 分頁)既未宣告也未渲染 `errors`,父層又沒有傳入 —— 驗證或儲存失敗被完全靜默吞掉。現在與設定編輯器一致,錯誤直接顯示在編輯區頂端。
- **分流隧道延遲探測目標誤判「不在 AllowedIPs 內」**:Wails IPC 序列化欄位為 snake_case(`peers` / `allowed_ips` / `public_key` / `interface.dns`),前端卻以 camelCase 讀取而取不到值,覆蓋檢查因而認為一個合法位址都沒覆蓋 → 任何 IP / 網域都被拒。同時修復詳情頁公鑰、DNS 顯示為空白的問題。
- **端點等資訊無法選取複製**:hero 端點從「點擊複製按鈕」改回可選取文字 —— 使用者通常只想要其中一段(只要主機名,或只要括號內的即時 IP),按鈕永遠只能給整串。並補上全域選取反白:應用程式全域停用文字選取,以往只有少數區域自行 opt-in,反白顏色隨系統、在深色卡片上幾乎看不見;現由一條全域規則統一使用主題強調色,輸入框 / 文字區也明確恢復可選取。

### 🔧 變更

- **隧道詳情頁精簡**:移除與 hero 重複的獨立端點顯示區塊;延遲結果卡與延遲探測目標輸入框合併為同一列;目標輸入框下方的長提示摺疊進「詳細說明」(常駐只留一句);「握手 / 時長 / 介面名稱」移入 hero 並加大字級;網域端點會在後方附加即時解析位址(`host:port (ip:port)`),取自探測串流而非前端另做 DNS,字面 IP 則不重複顯示。
...
- **隧道名稱校驗與淨化（共享模組）**：新增前端模組 `tunnel-name.js`，鏡像後端 `ValidateTunnelName()` 的全部規則（允許字元 `A-Za-z0-9-_ `、≤64 字元、禁首尾空格、禁 Windows 保留裝置名稱），供**匯入**、**設定編輯器名稱欄**、**詳情頁重新命名**三處錄入點共用，行為完全一致。
- **名稱自動修正提示**：名稱中的非法字元（如 `.`、`()`、`·`、中文等）會自動替換為 `-`，並以 toast 告知「原名 → 實際儲存名」，不再讓使用者誤以為寫入失敗。

### 🔧 變更

- **手動改名行為與匯入統一**：此前手動改名遇到非法字元會直接彈出後端英文報錯，而匯入則靜默改名，兩者行為不一致；現在**一律自動替換非法字元並提示**。淨化後仍無法成立的僅剩兩種情況，會明確報錯：輸入全為非法字元（改後為空）與 Windows 保留裝置名稱（`CON`/`NUL`/`COM1` 等）。
- **編輯器名稱欄回顯實際儲存名**：儲存後名稱欄顯示的是真正落盤的名字，避免「以為叫 A、實際存成 B」。
- **提示文案多語言化**：匯入成功的提示此前是硬編碼英文（`Imported "x"`），現統一走 i18n；檔名被修正時改用專門文案說明原因（5 語言齊備）。

### 🛠 內部

- **保留裝置名稱補充**：`reservedDeviceNames` 補入 `CONIN$` / `CONOUT$`（防禦性條目，`$` 本不在允許字元集內），並新增 `TestReservedDeviceNamesCoverage` 直接釘住該表，防止條目被誤刪而無測試報警。

## [2.1.2] - 2026-09-14

### 🐛 修復

- **macOS 下所有文字輸入框無法貼上 / 複製 / 剪下** — 先前的自訂選單列（`installCustomMenuBar`）只保留了 App 與 Help，捨棄了 Edit 選單。在 macOS 上，WebView 內的文字編輯（設定、欄位、腳本編輯器及一切輸入框）的 Cmd+X/C/V 等指令是經由選單列 Edit 選單的 responder 鏈路由到 WebView，缺少該選單會導致貼上 / 複製 / 剪下全部靜默失效。現已補回 `application.EditMenu` 角色，恢復全部文字欄位的貼上能力（僅影響 macOS；Windows / Linux 不受影響）。
...
- **手動連接 / 斷開與覆蓋鎖（Manual Override）**：主操作按鈕現在明確為手動動作，強制執行連接 / 斷開並忽略任何自動規則；隧道詳情顯示「控制來源」晶片——自動（規則 / 預設兜底）、手動斷開（規則已暫停）、手動連接（已鎖定），懸停查看完整語意。
- **預設兜底生效指示**：當沒有規則命中、收斂到預設狀態時，預設狀態行的外圈高亮與命中規則一致，並標註「目前生效：已執行預設兜底動作」。
- **隧道狀態術語釐清**：區分「已連接」（網卡就緒且握手成功、可傳輸資料）、「連接中」（網卡已建立但尚未握手，半開態）、「已斷開」（無網卡），並附詳細說明。
- **路由視覺化增強**：新增區域網直連路由（閘道為 `-`）過濾開關；蜂巢式（Cellular）介面類型標籤；說明彈窗解釋 `-`、ARP / 鄰居項目的自動隱藏、虛擬 / 第三方網卡顯示建立軟體名。
- **DNS 洩漏測試韌性**：公共 DNS 清單來源（public-dns.info）取得失敗時自動回退到內建公共 DNS 清單，測試不受影響，並給出清晰提示。
- **自動化規則「生效」判定說明**：詳細說明命中需同時滿足的 4 個條件（條件符合目前網路、為清單首個匹配、引擎實際執行動作、隧道真實狀態與動作一致），並釐清 disconnect 正確執行後隧道即 down 屬「生效」、半開態不冒稱生效。

### 🔧 變更

- **隧道詳情主操作重構**：連接按鈕增加品牌酒紅左側條（#A01D21，呼應 App 圖示）；主 CTA 加大並加重陰影；斷開按鈕常駐淺紅描邊；主操作 sticky 常駐，長面板下不滾出視野；雙行副標題移入 tooltip。
- **主題與對比度**：深色 `--text-muted` 對比度提升至 AA（約 4.8:1）；深色 `--green` 對齊圖示醫用綠（#22c55e，AAA）；清理冗餘 token（`--blue-tint`、`--accent-blue`），統一引用 `--accent-tint`。
- **文件**：README 不再推薦 WireTunnels；移除 2.0「Windows 系統服務」路線圖（該目標不再納入規劃）。

### 🛠 內部

- **新增測試**：手動覆蓋鎖（manual override）、介面歸屬（iface owner）、DNS 洩漏公共清單取得；更新自動化評估測試以涵蓋新語意與預設兜底。
- **多語言檔案 515 鍵對齊**（zh / en / ja / ko / zh-TW）。

## [1.8.5] - 2026-09-12

### ✨ 新增

- **策略層（Policy Layer）**：基於隧道快照的純函式分析器——前綴分類、路由嚴重度、DNS 與保護檢查。
- **衝突策略**：儲存時允許並記錄日誌；手動連接時阻斷並彈出衝突警告；自動化觸發時阻斷並通知；絕不改寫 AllowedIPs。
- **每隧道策略**（存於 `.meta` 側車檔）：traffic_protect、domains、use_as_default_dns、dns_resolve_path、system_dns，並新增 TunnelPolicies 介面。
- **路由資料契約**：僅 L3 下一跳的閘道（直連為 `—`，MAC 與 ARP / 鄰居項目已過濾）、規範化目的地、僅單播。
- **診斷增強**：自動化網路板顯示介面類型標籤；更豐富的連接 / 斷開日誌；緊湊的隧道編輯器佈局。

### 🔧 變更

- **移除全域 Kill Switch**：從設定 / IPC / CLI / helper 中移除全域 Kill Switch，相關保護由每隧道策略接管。

## [1.8.0] - 2026-09-09

### ✨ 新增

- **執行日誌大幅充實** — GUI 啟動時記錄版本與平台；隧道的連線/斷線請求與結果（失敗原因以 WARN 級別記錄）、設定匯入（含 zip 批次匯入的成功/失敗統計）、儲存、刪除、重新命名都有對應日誌；自動化引擎逐隧道記錄決策（連線、斷線、略過及原因），手動斷線設定的「閂鎖」在設定與清除時都會提示，排查「規則為什麼不連」不再靠猜。
- **路由表閘道顯示「鏈路直連」** — 閘道為 `link#N` 的鏈路路由不再顯示晦澀的核心記號，統一顯示為本地化的「鏈路直連」（懸停仍可查看原始值）。

### 🔧 變更

- **儲存自動化策略立即生效** — 在自動化編輯器儲存規則或預設狀態後，應用會立刻觸發一次重新評估，不再需要等待下一次網路事件。
- **日誌增加類別欄位** — 日誌條目現在帶 `category`（tunnel / network / app），便於按主題篩選。

### 🐛 修復

- **AllowedIPs 衝突檢測誤報徹底修復**（1.7.9 修復不完整的補充）：
  - 隧道自身介面現在**按位址識別**（介面攜帶隧道自己的 Address 即跳過），不再依賴 helper 狀態回傳介面名稱——CLI 的 `connect` 預檢與 GUI 狀態查詢失敗的路徑也不會再把隧道自己的路由當成衝突源。
  - 新增 CIDR **嚴格位於已有更寬路由內部**時不再警示：最長前綴匹配下更具體的路由永遠勝出、結果確定無歧義。此前 Clash/Mihomo TUN 的近預設路由（如 `8.0.0.0/5` 罩住整個 `10.0.0.0/8`）會把所有私有網段都報成「衝突」。相等前綴（兩隧道搶同一段）與新增超集（全隧道覆蓋既有路由）仍會照常警示。
- **只有預設狀態、沒有規則的隧道此前從未被評估** — 引擎只遍歷「有規則」的隧道，導致 1.7.9 的「預設狀態單獨生效」實際不生效：預設連線的隧道在無規則命中時不會自動連線。現在預設狀態與規則的聯集都會被評估。
- **路由表顯示修復** — macOS 省略寫法的目的地現在展開為完整 CIDR（`2/7` → `2.0.0.0/7`）；目的與閘道欄的欄寬修正，超長位址不再溢出混入相鄰欄或被截斷（改為省略號 + 懸停顯示完整值）。
- **欄位編輯器 Endpoint 標記必填** — Endpoint 現在與其餘必填欄位一樣帶紅色 \* 標記。

### 🛠 內部

- 新增衝突檢測迴歸測試（子網抑制 / 相等前綴 / 新超集 / 實機驗證）、macOS 路由目的地展開測試與自動化評估觸發測試；多語言檔案 456 鍵對齊。

## [1.7.9] - 2026-09-08

### ✨ 新增

- **工具列選單顯示版本號** — 選單第一行「WireGuide Plus」後面現在會帶上目前版本號，一眼確認正在執行的建置。
- **欄位編輯器必填項標記** — PrivateKey、Address、PublicKey、AllowedIPs 等必填欄位現在帶紅色 \* 標記，懸停顯示本地化提示。

### 🔧 變更

- **預設狀態單獨即可生效** — 只設定預設狀態、不新增任何規則的隧道，現在會**始終收斂到該預設狀態**（此前這種設定會被完全忽略）；「規則與預設狀態都沒有」的隧道依然完全不介入。相應地，`automation default` 不再要求隧道先有規則，把規則刪光也會保留預設狀態。
- **編輯器版面優化** — 設定編輯彈出視窗加大（860×620 → 900×760），設定編輯區更寬裕；腳本編輯器的四個鉤子改為兩欄排列（PreUp+PostUp 一行、PreDown+PostDown 一行），面板整體更緊湊；欄位編輯器標籤欄加寬，PersistentKeepalive 不再與輸入框重疊。

### 🐛 修復

- **修復 macOS 開機自啟動失效** — launchd 為登入啟動項提供的環境變數裡沒有 `HOME`，應用被拉起後立刻因無法定位資料目錄而退出（plist 確實被觸發了，但程序秒退）。路徑解析現在回退到系統帳戶資訊（getpwuid），不再依賴環境變數；LaunchAgent 同時顯式注入 `HOME`、宣告為 Aqua 工作階段的互動式程序；取消自啟動時先註銷再刪除。
- **預設連線的隧道開機後仍不自動連線** — 啟動時現在總會投遞一次自動化評估；但網路尚未識別（無 SSID 且無實體網卡位址）時不憑空動作，等 SSID 上報或路由事件到達後，再按真實條件完成決策。開機自動連線依然生效，且每一個決策都經過條件判斷。
- **修復 AllowedIPs 衝突檢測誤報** — 隧道已連線時不再把**自己的介面**當成衝突來源（此前每次檢查都會把全部 CIDR 報成與自己重疊）；網段重疊判定收緊為嚴格相交，兄弟前綴（如 `10.30.30.0/24` 與 `10.30.35.0/24`）不再被誤判為包含衝突，跨位址家族（IPv4 vs IPv6）也不再比較。

### 🛠 內部

- 測試腳本與文件同步已移除的「否則」條件與預設狀態語義；新增網段重疊迴歸測試與引擎預設態單元測試。

## [1.7.8] - 2026-09-08

### ✨ 新增

- **自動化規則改為「預設狀態 + 有序規則」** — 每條隧道現在是一份按優先順序排列的規則清單：自上而下逐條比對，**首個命中的規則生效**，規則內多個條件為「且」關係；所有規則都不命中時收斂到該隧道的**預設狀態**（連線 / 斷線），不再有「否則（Otherwise）」分支。預設狀態在自動化編輯器頂端設定，規則卡可拖曳調整優先順序，動作徽章點一下即可在連線 / 斷線之間切換。
- **依期望狀態收斂（Desired State → Reconcile）** — 引擎現在持續把每條隧道推向它應有的狀態：命中「連線」就連線，命中「斷線」就斷線，與隧道當初是如何建立起來的無關；沒有設定任何規則的隧道依然完全不介入。
- **命令列新增 `automation move` 與 `automation default`** — `wireguideplus ctl automation move <隧道> <原序號> <新序號>` 調整規則優先順序，`wireguideplus ctl automation default <隧道> <connect|disconnect>` 設定預設狀態。

### 🐛 修復

- **網路事件風暴不再重複觸發自動化** — 一次 Wi-Fi 連線產生的多個 SSID / 路由事件會合併成一次評估：觸發源只投入單槽信箱，評估開始時總是重新取樣最新的網路內容，評估期間抵達的事件最多只保留一個並在結束後再跑一次。既不會積壓，也不會拿過期的網路狀態做決策。
- **結束時的自動化收尾競爭** — 關閉應用後仍處於待處理狀態的評估請求不再執行，避免它與「中斷所有隧道」的清理流程爭搶同一條隧道。
- **舊設定的「否則」規則自動遷移** — 讀取舊設定時，第一條有效的「否則（none_match）」規則會被轉換成該隧道的預設狀態，排在它後面、被它完全遮蔽的規則一併移除；沒有任何「否則」規則的舊設定遷移為預設「斷線」。遷移具冪等性，結果與原先行為一致。

### 🛠 內部

- **評估動作抽取為純函式並補充冪等性測試** — 同一網路內容連續評估兩次不得產生第二次狀態變更動作；被手動關閉的隧道在任何路徑下都不會被自動重連。
- **設定遷移 / 命令列 / 介面三條寫入路徑一致性測試** — 三條路徑落地後必須得到同一份正規化模型：無 `none_match` 殘留、有規則必有合法預設狀態、無規則必無預設狀態。
- **文件同步** — 設計文件補齊預設狀態與有序規則語義、事件合併機制與鎖序說明；手動測試清單移除已廢棄的「否則」用例，新增事件風暴與關閉時序兩項驗證。

## [1.7.6] - 2026-09-07

### 🐛 修復

- **新建隧道時立即顯示出口綁定面板** — 開啟「固定介面」後，新建隧道的編輯視窗此前要等輸入隧道名稱後才出現綁定選項（面板顯示條件被隧道名誤攔截）；現在開啟視窗即顯示，所選出口在儲存成功後寫入，失敗不影響隧道本身的建立。

## [1.7.5] - 2026-09-07

### ✨ 新增

- **隧道腳本（PreUp / PostUp / PreDown / PostDown）編輯器** — 隧道編輯視窗新增腳本面板：可為每個鉤子選擇既有腳本檔案（不限目錄）、新建空白腳本（預設放在 scripts 目錄，可重新命名）、直接編輯程式碼或一鍵清除；寫入採用純文字改寫，不破壞 conf 中的註解與手工排序。
- **獨立 scripts 資料夾與設定匯入/匯出** — 新增與 tunnels、logs 並列的 scripts 資料目錄；設定新增「匯出設定」（打包 tunnels + scripts + config.json，不含日誌）與「匯入設定」，方便整機遷移。
- **欄位編輯檢視** — 隧道編輯視窗新增 conf / 欄位雙頁籤：欄位頁按 conf 鍵逐一提供輸入框（interface / peer 分組），AmneziaWG 混淆參數僅在其開關開啟時顯示；儲存時仍寫回 conf 文字，註解與自訂行原樣保留。
- **每隧道實體出口綁定（Windows / Linux / macOS）** — 開啟 Pin Interface 後，可在隧道編輯中將該隧道的加密流量固定到指定實體網卡：Windows 使用 socket 層級 IP_UNICAST_IF 綁定，Linux 使用 `ip route … dev <網卡>` 旁路路由，macOS 旁路路由附加 `-ifscope`；綁定失敗即連線失敗，不回退預設路由，避免流量外洩。
- **綁定網卡失效處理彈窗** — 綁定的出口網卡遺失時不再靜默回退：彈窗提供「保持綁定等待恢復」「自動切換到可用網卡」「開啟隧道編輯手動指定」三種處理方式，且每次失效事件只提示一次。
- **AmneziaWG 開關二次確認與統一徽章** — 設定中開啟 AmneziaWG 支援時要求與 Pre/Post Scripts 相同的高亮二次確認；隧道清單與詳情頁的徽章統一為 AmneziaWG ON（紫）/ AmneziaWG OFF（紅）兩種樣式。

### 🐛 修復

- **網卡清單改用通用名稱** — Windows 網卡下拉此前顯示硬體描述（如 Realtek…），現優先顯示系統通用名稱（乙太網路、WLAN 等），硬體型號作輔助資訊；Linux 按 Wi-Fi / Ethernet / 行動網路分類，並修正預設路由判定導致所有網卡都被標為預設出口的問題；macOS 讀取系統網路設定中的連接埠名稱。
- **欄位編輯器無法開啟** — 頁籤切換參照了未定義的變數，點擊「欄位編輯」直接報錯；已補齊宣告並在切換時按目前 conf 文字重新解析。
- **AmneziaWG 徽章首次進入誤顯示 ON** — 徽章此前按預設值渲染、未等待設定載入完成；現嚴格以已載入的設定為準，讀不到開關狀態時不渲染。
- **Pin Interface 誤報「目前平台不支援」** — 有作用中隧道時切換開關會被誤判為平台不支援並回溯；現在作用中隧道保持目前綁定、重新連線後生效，僅記錄日誌。
- **Linux 出口綁定不再回退預設路由** — 旁路路由安裝失敗時連線直接失敗，避免流量走預設出口造成外洩。

### 🛠 內部

- **視窗統一支援拖曳與縮放** — 設定 / 自動化 / 隧道編輯視窗可按標題列拖曳位置、右下角自由縮放；編輯視窗預設尺寸加大、右上角新增關閉按鈕，「conf 文字」頁籤更名為「conf 編輯器」，欄位頁按鈕樣式與 conf 頁統一。
- **欄位編輯分組全寬顯示** — interface / AmneziaWG / peer 分組統一填滿編輯區寬度，長值不再被擠在半欄。
- **本機建置補齊圖示與版本資源** — 手動建置時按 CI 同款流程產生資源檔案，測試套件不再缺圖示；正式發布仍由 CI 建置。

## [1.7.1] - 2026-09-06

### ✨ 新增

- **舊版本資料提醒改版** — 偵測到舊版本（儲存於 "wireguide" 目錄）遺留的資料時，改為彈出對話框讓使用者自行選擇：開啟舊資料夾或新設定資料夾檢視、移動到新的設定資料夾、不再提醒，或本次取消並在下次啟動時再次提醒。
- **移動時的同名檔案衝突提示** — 目標設定資料夾已存在同名檔案時，對話方塊會列出衝突檔案，並提供「覆蓋並移動」「開啟新資料夾檢視內容」「取消且不再提醒」「本次取消、下次再提醒」四種處理方式。

### 🐛 修復

- **略過遷移時舊資料被誤刪** — 先前每次呼叫遷移都會無條件清理舊目錄，導致選擇「稍後再看」或因同名衝突被略過時，尚未遷移的舊資料被悄悄銷毀，且之後再也掃描不到。現在只有在全部檔案真正移動完成（無任何略過）後，才會清理舊目錄並停止提醒。

### 🛠 內部

- **移除設定中的舊版本資料遷移入口** — 舊版本檔案偵測僅在軟體首次執行時自動進行，無需在設定中手動重新偵測；對應的設定區塊、後端重置介面及其狀態儲存一併移除。
- **遷移相關文案全語言補齊** — 新增的衝突提示與操作按鈕已補齊 5 種語言。
