...
// IP and route metric. Returns (0, 0, nil, 0) if no default route is installed (e.g. boot‑before‑DHCP)
// or all default routes are owned by excluded interfaces.
//
...
func DefaultRouteV4LuidAndIndex(excludedAliases []string) (uint64, uint32, net.IP, uint32) {
...
func DefaultRouteV6LuidAndIndex(excludedAliases []string) (uint64, uint32, net.IP, uint32) {
	excludedLuids := make(map[uint64]struct{}, len(excludedAliases))
	for _, alias := range excludedAliases {
		if luid, ok := convertInterfaceAliasToLuid(alias); ok {
			excludedLuids[luid] = struct{}{}
...
func HasDefaultRouteOnInterface(family uint16, ifIndex uint32) (has bool, metric uint32) {
	...
	for i := 0; i < n; i++ {
		...
		if family == afInet {
			ip := prefix.Prefix.ip()
			if ip != nil && ip.Equal(net.IPv4zero) && prefix.PrefixLength == 0 {
				isDefault = true
			}
		} else {
			ip := prefix.Prefix.ip()
			if ip != nil && ip.Equal(net.IPv6zero) && prefix.PrefixLength == 0 {
				isDefault = true
			}
		}
		if !isDefault {
