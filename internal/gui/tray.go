...
// the status-driven scheduleRebuild path: each tunnel is a checkbox whose
// checkmark and emoji are refreshed on every rebuild, and clicking the
// checkmark toggles the tunnel via ctx.IsChecked(), so a ≤1s-stale
