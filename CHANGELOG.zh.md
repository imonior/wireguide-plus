...
### ✨ 新增
- Windows：缺少 WebView2 运行时时弹出原生下载提示窗口（含下载链接），不再静默启动失败。

### 🔧 變更
- Windows 安装包不再捆绑 Microsoft Edge WebView2 引导程序，改由系统已安装的运行时加载。

### 🛠 内部
- 发布流程在推送 Homebrew cask 前先 rebase，避免共用 `imonior/homebrew-tap` 的其他项目并发改动导致推送失败。

## [2.2.10] - 2026-09-20

### 🐛 修复

- **系统栏图标可能在后台进程仍在运行时悄悄消失**：Windows 上 Wails 的托盘会先替换图标句柄、再通过系统 Shell 重新应用；该处一次瞬时的 `Shell_NotifyIcon` 失败会抛出 panic 并被吞掉，留下一个已失效的句柄——图标消失而进程依然存活。触发场景包括自动深色模式切换、显示器休眠/唤醒、远程桌面重连或快速用户切换（`WM_TASKBARCREATED` 只能覆盖资源管理器重启）。现加入 60 秒心跳重新应用当前连接态图标，且所有图标更新串行化以免句柄交换竞态；某次失败仅在下个周期重试。

### 🛠 内部

- **托盘图标更新串行化并加 panic 保护**：`safeSetIcon` 从 Win32 失败中恢复而非让调用方死锁，瞬时错误不会再冻结保存/状态流。

## [2.2.9] - 2026-09-19

### 🐛 修复

- **编辑保存可能在落盘前静默中断**：保存链引用了一个从未导入的辅助函数（`sanitizeTunnelName is not defined`），报错完全不达界面——不弹提示、不记日志，按钮点了没反应。已修复缺失的导入，保存链上的任何意外错误都会被捕获并明确提示，不再无声消失。
- **保存／导入失败提示是英文且日志无痕迹**：配置校验消息此前是裸 Go 字符串，现已改为错误码驱动并在五种语言中本地化。导入（zip / 二维码 / 文件）、导出与配置读取的失败分支都会写日志，被拒绝的配置从此可追溯。
- **保存卡死会可见地失败，而非永久挂起**：编辑保存链的每一步（校验 → 断开 → 写入 → 重连）都有 20 秒看门狗兜底，超时给出本地化提示，按钮不再僵死。
- **状态探测失败不再阻断保存**：保存期间的连接状态探测不可达时，此前会让整个保存以英文报错收场。
- **每次保存后出现 4 条假的「rpc timeout」日志**：20 秒看门狗定时器在调用成功后没有取消，到点照常触发。现在调用一结束定时器即被清除。

### 🛠 内部

- **保存链每一步都留面包屑日志**：前端步骤通过新增的 `LogFrontend` 绑定写入日志，webview 的未处理错误与 Promise 拒绝也被捕获进同一份日志，配置更新入口会记录每次调用——静默保存从此可以从最后一条面包屑定位断点。

## [2.2.8] - 2026-09-19

### 🐛 修复

- **连接状态下编辑隧道保存无反应（Windows 与 macOS 均存在）**：后端会拒绝向已连接隧道写入配置，且失败被静默吞掉——既不记日志也不弹提示，于是「看起来保存成功、实际仍是旧配置」。现在连接中也可编辑：保存新配置时会先断开隧道、写入改动、再自动重连。三种结果都会明确提示——「已保存并重新连接」／「已保存但重连失败」／「保存失败，已用旧配置重启」，不再静默失败。
- **延迟探测数据在隧道间串台**：只有一个隧道在线时，选中另一条隧道会显示主隧道的延迟与解析 IP（helper 仅在多于一个隧道在线时才填充各隧道统计，详情页在匹配不到选中隧道时回退到了全局状态）。现在探测行严格按选中隧道匹配，匹配不到就一条都不显示；只展示真正解析出的 IP，绝不回显来自其它隧道的原始目标。
- **上游已断仍显示「已连接」且握手正常**：Wi-Fi 或物理网卡还在，但路由器丢了上行时，隧道仍报「已连接」、握手时间也正常。现在状态新增「握手陈旧」标记——仅当已连接超过阈值、且该时间窗内没有新握手时才置位，刚重连的隧道不会被误标；hero 圆点与握手行转琥珀色，列表圆点同步。
- **窄窗口下探测两列错行**：左侧「预设」列标题把右列输入框顶下一行。已去掉两列标题，左右行对齐。

### 🛠 内部

- **发布说明重新由人工维护的 CHANGELOG 生成**：发布体取英文默认的 `CHANGELOG.md` 中最新的 `## [x.y.z]` 段落，再追加来自 `build/release-tail.md.tmpl` 的下载／校验／Full Changelog／SignPath 页脚——恢复 v2.2.7 之前每个版本一贯的页面结构。已移除 git-cliff（及 `cliff.toml`），发布体随 CHANGELOG 而非提交信息生成。

### 📝 文档

- **CHANGELOG 改为英文默认**：`CHANGELOG.md` 现为英文（与 `README.md` 命名约定一致），简体中文版移至 `CHANGELOG.zh.md`。
