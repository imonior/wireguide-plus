...
- **状态气泡现在跟随你所选的语言。** Windows 的 GDI 气泡只认显式语言代码，「自动」落到英文兜底分支，而主界面早已按系统语言本地化——中文、日文、韩文系统上的通知因此一直是英文。现在气泡与应用其余部分按同一规则解析「自动」：Windows 读取系统 UI 语言（并用语言标识的子语言区分简体/繁体中文），macOS/Linux 气泡在每次事件中收到解析后的语言并立即应用。繁体中文也第一次拥有了真正的繁体文案（已連線 / 開啟主視窗）与合适的字体——你看到的每一处文字都与设置里的语言一致。

## [2.4.1] - 2026-09-26

### 🐛 修复

- **同一 SSID 下的网络切换不再违背自动化规则。** 重新配置 Wi-Fi 网络（如静态 IP 改 DHCP）会瞬间抖断网卡，接口可能先带着可用地址恢复、SSID 报告却尚未跟上。在这个窗口里，以 SSID 为条件的"断开"规则无法匹配（暂时为空的）SSID，隧道的默认状态便悄悄接管，把规则要求保持断开的隧道连了起来——而 macOS 没有定时轮询兜底，错误连接可能一直维持。现在引擎对"半份上下文"失败关闭：规则依赖 SSID 的隧道，在其网络 SSID 未知期间整轮跳过，默认状态绝不越权替引擎判定它无法评估的条件。SSID 恢复已知的瞬间决策随即恢复——新的 SSID 报告本身就会触发评估，任何执行或跳过之后数秒还有一次性的沉降复查，重连监视器的策略门同样拒绝这半份上下文，抖动期间的盲恢复不再能做出引擎本不会做的决定。
- **macOS 图标链路补齐，新圆角图标出现在所有出货位置。** 菜单栏托盘图标、App 包图标（Dock / 启动台）与 DMG 安装包图标此前仍是旧的方角素材；现在包括 Windows 圆角 `.ico` 在内，全部由单一源文件 `build/appicon.png` 生成。
...
- **日志查看器一键打开日志目录** — 工具栏新增「打开日志目录」按钮，直接在 Finder / 资源管理器 / 系统文件管理器中打开日志所在文件夹（与设置页日志条目同源）。
- **托盘菜单状态符号** — 已连接隧道显示 🟢、未连接显示 🔴（macOS/Linux；Windows 托盘菜单由 GDI 绘制、无法渲染彩色 emoji，改用单色 ✔ / ✗）。
- **三平台统一的托盘点击** — 单击（左键或右键）打开托盘菜单，左键双击打开主窗口。

### 🔧 变更

- **状态通知改为每隧道一行，三平台统一：**「🟢 Connected: 名称」/「🔴 Disconnected: 名称」/ 🟡 连接中，整行按状态着色，取代原来的合并标题行。
- **状态气泡常驻直到处理** — 不再按时钟自动消失：由你手动关闭、打开主窗口、点断开，或被更新的事件替换；此前离开电脑时的一条断开警告可能在十秒后悄然蒸发。设置中的通知时长现在只作用于启动总览气泡。
- **macOS 气泡只浮出、不抢焦点** — 通过提升窗口层级浮在其他应用之上（前台是全屏应用时也可见），不再激活应用，通知出现在你正在做的事情*上面*，而不是*取代*它。

### 🐛 修复

- **网络瞬断不再违背自动化策略盲重连** — 网卡瞬时掉线（例如改静态 IP 会闪断一下）时，断线监视器过去会把缓存的隧道无差别全部恢复，抢在每隧道自动化规则之前。现在两条恢复路径都会先查询同一策略：规则要求断开的、网络未识别的、以及被暂停/豁免的隧道保持不动。

## [2.3.10] - 2026-09-24

### ✨ 新增

- **地址冲突会暂停该隧道的自动化，直到你做出决定。**当自动连接遇到已被其他客户端占用的地址时，现在会弹出常驻对话框：指明冲突来源软件、显示该隧道的自动化状态，并给出仅有的两个选择——停止本隧道自动化，或继续尝试。没有「稍后处理」的退路：在你答复之前，helper 会挂起该隧道的自动连接；未处理的冲突会在每次 GUI 启动以及 helper 重启后再次弹出。
- **启动总览气泡。**每次 GUI 启动 15 秒后，弹出一个列出全部隧道的气泡：每条隧道的实时状态与自动/手动模式，取代原先启动后 10 秒的状态气泡。

### 🐛 修复

- **失败的连接尝试不再产生「已断开」幻影气泡。**只有真正连接过的隧道才会被播报为断开；「连接中→失败」不再被误报为断开，而真正的重新连接仍会播报。

### 🔧 变更

- **状态气泡在各平台采用统一语汇：**已连接＝绿色对钩＋绿色文字，断开＝红色叉＋红色文字，连接中＝黄色圆点——Windows 原生气泡以矢量绘制，macOS/Linux 气泡同样如此，启动总览各行也一样。
...
- 新增 `tools/checkrelease`：只读校验，确认五份 i18n 语言 JSON 拥有完全一致的键集合，且五份 CHANGELOG 都包含本次发布的版本小节。`bump:version` 现在会先运行它，确保发布不会带着翻译漂移或缺失某种语言的更新日志出厂。

### 🔧 变更

- 新增统一的本地质量门禁（`task check`：`go vet` + 针对 `./internal/...` 的竞态测试），并让 CI 在直接推送到 `main` 时也会运行，堵住「推到 main 完全不测试」的窗口。
- CI 现在会重新生成 Wails bindings，并与已提交的 `frontend/bindings` 比对，一旦发现漂移即失败，防止前端/后端 IPC 调用静默错位。
...
- **隧道详情页新增「停止自动化」开关**: 一个醒目的单击开关，与设置页的「排除出自动化」选项作用相同——开启后引操不会再连接或断开该隧道，直到你关闭它。
- **全局「禁止系统休眠」（保持后台运行）**: 新的可选设置，应用运行时让计算机保持唤醒，使空闲/休眠的电源策略不会默默杀掉隧道。默认关闭——除非你主动开启，否则不覆盖系统电源策略。Windows 上调用 `SetThreadExecutionState`。
- **交互式地址冲突弹窗**: 当另一个 VPN 客户端已占用你的某个隧道地址时，应用会点名冲突的软件和适配器，并提供「停止该隧道自动连接」的选项。由你决定——它绝不会强行停止另一个客户端。

### 🐛 修复

- **状态栏重复播报已连接的隧道**: 连接提示现在只上报状态真正发生变化的隧道（已连接/已断开），因此持续在线的隧道只会被播报一次，而不会在每个 30 秒状态轮询时反复弹出「X 已连接」。这正是「TS453Dmini 已连接」反复弹出的根因——真正在抖动的是另一条隧道，而弹窗只是把当时仍在线那条的名字报了出来。
- **手动「停止自动化」现在覆盖所有自动重连**: 失效连接的监控器现在同样遵循每隧道的排除设置，因此关闭某隧道的自动化会真正阻止它被自动重连（此前监控器仍可能在握手超时时重连它）。

### 🔧 譪更

- **移除了旧的基于签名的提示去重**: 它只是把气泡藏起来，掩盖了真实的连接变化。现在改为在源头通过上报真正的每隧道状态变化来减少干扪。

## [2.3.2] - 2026-09-21

### ✨ 新增

- **将隧道排除出自动化**：隧道策略面板新增「排除出自动化」开关，开启后引擎永远不会连接或断开该隧道，但保留其规则与默认状态。适用于同一条隧道同时被另一个 WireGuard 客户端控制的情况——手动覆盖只能维持到下次重启，而这个开关会一直生效。

### 🐛 修复

- **自动化每 30 秒硬撞隧道并狂刷状态栏**：当连接失败——最常见原因是另一个 WireGuard 客户端已经占用了该隧道的 `Address`（Windows 报 `The object already exists`）——协调器在每个轮询周期都会无退避地重试，反复拆建 Wintun 适配器，并每轮重新弹出连接气泡。现在失败的自动化连接会按退避节奏重试（30 秒 → 1 分 → 2 分 → 5 分，封顶），并在隧道转为活跃、设置了手动闩锁或网络规则变更时解除限流。
- **同一状态反复弹出「已连接」气泡**：托盘现在按签名对状态通知去重——已经播报过（或刚被抑制）的状态不再重复显示，且首次抑制会作为 Info 记入日志，而不是再次弹窗。
- **连接失败在分配地址一步报得莫名其妙**：`AssignAddress` 现在把「地址已在本接口上」视为成功（冪等）；当地址确实被占用时，会在错误中点名冲突适配器，让你知道是哪个别的客户端在跑同一条隧道。
- **启动时会警告同一条隧道在别处运行**：启动瞬间 helper 会拿每条隧道的 `Address` 与机器真实接口逐一比对（排除自身适配器），若发现被别的客户端占用，则在日志中点名冲突适配器并给出停止服务的提示。

### 📝 文档

- README 新增「一条隧道，一个客户端」规则说明与按隧道「排除出自动化」开关介绍。
## [2.3.1] - 2026-09-21

### ✨ 新增

- **熄屏、锁屏或睡眠时保持连接**：新增「空闲时保持连接」设置（默认开启，可按隧道覆盖），隧道连接时强制启用持久 keepalive，使其在熄屏、屏保和锁屏下不再悄然掉线。系统睡眠后唤醒会自动重新探测并重连。Windows 现在除系统睡眠外还监听显示器电源与会话锁屏/解锁，屏保或锁屏不再留下死掉的隧道。

### 🐛 修复

- **在已安装 WebView2 的机器上误报「未安装」（Windows）**：启动检查只读了 64 位注册表视图，而 Evergreen 运行时实际注册在 32 位视图（`WOW6432Node`），于是 64 位程序总是判为缺失、弹出错误的「下载 WebView2」提示，点取消还会直接退出应用。现在探测两个注册表视图 × 两个根键，并加上磁盘安装目录兜底；另加一层「启动后真失败」的兜底。
- **隧道已死却显示不断增长的连接时长**：连接时长按挂钟时间累加，机器睡眠或屏保令握手停止后计时仍在爬升——实际半夜已断的隧道会显示「22 小时」。现在对端无响应期间时长冻结、恢复后续算，详情页也会标注该冻结状态。

### 🔧 變更

- **发布产物文件名带上版本号**：安装包、便携压缩包以及 `.deb` / `.dmg` 都内嵌发布版本（如 `wireguideplus-2.3.1-amd64-installer.exe`），下载文件即可辨识版本，同目录下多版本不再互相覆盖。更新器本就按文件名 token 匹配，因此旧版本仍能正常自动更新。

### 🛠 内部

- **空闲会话监听成为跨平台统一接缝**：`SessionDetector` 在 Windows 上报唤醒（亮屏 / 解锁）事件，其他平台为空实现，接入既有重连监控——且只对已断开或握手陈旧的隧道重连，健康的隧道不会被扰动。
