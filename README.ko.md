...
- **지연 프로브** — 터널별 지연 프로브가 다중 대상 병렬을 지원합니다. 각 후보(공용 리졸버 8.8.8.8 / 223.5.5.5, 터널 엔드포인트, 임의의 사용자 지정 주소 또는 도메인)를 동시에 프로브하고 해석 IP·종류·왕복 시간을 표시합니다. 분할 터널에서는 사용자 지정 대상이 저장 전 터널의 AllowedIPs 커버리지로 검증됩니다.
