...
이 설정은 터널 상세 보기 상단의 원클릭 "자동화 중지" 토글에서도 할 수 있어, 터널을 보는 그 자리에서 자동화를 억제할 수 있습니다. 전역 "시스템 절전 방지" 설정(기본값 꺼짐)은 터널이 연결된 동안에만 컴퓨터를 깨운 상태로 유지하며(Windows·macOS·Linux 모두 지원), 마지막 터널이 끊기면 즉시 해제되어 유휴/절전 전원 정책이 터널을 조용히 끊지 못하게 합니다. 다른 클라이언트가 이미 터널 주소를 점유한 경우, WireGuide Plus는 충돌 소프트웨어를 명시하고 "해당 터널 자동 연결 중지"를 제안하는 대화형 대화상자를 표시합니다——결정은 당신이, 다른 클라이언트를 강제 중지하지 않습니다.
