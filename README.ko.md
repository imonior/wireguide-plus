...
- **트레이 연결 상태 알림** — 시작 후 15초(권한 상승 확인 후)에 모든 터널의 개요 버블을 표시하여
  터널별 실시간 상태와 자동/수동 모드를 알려주며, 설정한 시간(기본 10초, 설정에서 조절 가능) 후 자동으로 닫힙니다.
  이후 네트워크 변화(Wi-Fi 전환, 랜 케이블 분리, 인터넷 끊김 등)로 터널 상태가
  바뀌면 터널별 1줄 상태 버블(「🟢 Connected: 이름」초록 / 「🔴 Disconnected: 이름」빨강 / 🟡 연결 중 주황)을 표시합니다.
  버블은 포커스를 빼앗지 않고 다른 앱 위에 뜨며 **사용자가 처리할 때까지 유지**됩니다(닫기 / 메인 창 열기 / 연결 끊기, 또는 새 이벤트로 교체).
  버블은 작업 영역 구석에 자리해 콘텐츠를 가리지 않으며, 드래그로 어디서나 이동할 수 있습니다.
  세 플랫폼 모두 동일하게 작동합니다(이전에는 macOS/Linux에 버블이 없었습니다).
- **통일된 트레이 동작** — 왼쪽이든 오른쪽이든 한 번 클릭하면 메뉴가 열리고, 왼쪽 두 번 클릭으로 메인 창을 엽니다(3 플랫폼 공통).
  메뉴 항목은 macOS/Linux에서 🟢 / 🔴 상태 기호를 표시합니다(Windows 메뉴는 GDI 렌더링이라 컬러 이모지를 쓸 수 없어 ✔ / ✗ 사용).
...
4. **시스템 통합** — 로그인 시 자동 시작, 트레이 최소화 시작, 트레이 연결 상태 알림
   (시작 후 15초 전체 터널 개요, 표시 시간 조절 가능 / 상태 변경 버블은 포커스 없이 처리까지 유지),
   통일된 트레이 클릭(클릭=메뉴, 두 번 클릭=메인 창).
5. **창 제목 및 상호작용 개선** 등.
...
- **SSID 미확인 시 실패 닫기**: 현재 네트워크의 SSID가 잠시 모르는 상태라면——Wi-Fi 인터페이스가 흔들림에서 먼저 복구됐지만 신원 재보고가 아직 따라오지 않은 경우(같은 SSID로 재설정할 때의 상황)——SSID 조건인 규칙은 **판정 불가**로 다루어 그 터널은 이번 라운드를 건너뜁니다. 엔진이 아직 판단할 수 없는 조건에 기본 상태가 몰래 우선하지 않으며, SSID가 다시 알려진 순간 평가가 즉시 재개됩니다.
...
로그 뷰어 도구 모음과 설정의 로그 영역 모두에 '로그 폴더 열기' 버튼이 있어 시스템 파일 관리자에서 이 폴더를 바로 열 수 있습니다.
