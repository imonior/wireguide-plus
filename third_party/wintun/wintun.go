...
// wintunDLLName returns the Wintun DLL filename for the current GOARCH.
// The upstream wintun-go hardcodes "wintun.dll"; WireGuide Plus instead
// ships one DLL per architecture (wintun-amd64.dll / wintun-x86.dll /
// wintun-arm64.dll) so a single portable bundle works on every machine
// and the helper picks the right driver without renaming files.
...
var (
	modwintun                         = newLazyDLL(wintunDLLName(), setupLogger)
