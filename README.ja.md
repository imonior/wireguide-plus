...
- **遅延プローブ** — トンネルごとの遅延プローブが複数ターゲットの並行に対応。各候補（公共リゾルバ 8.8.8.8 / 223.5.5.5、トンネルエンドポイント、任意の手動アドレスまたはドメイン）を同時にプローブし、解決 IP・種別・往復時間を表示します。分割トンネルでは手動ターゲットは保存前にトンネルの AllowedIPs カバレッジで検証されます。
