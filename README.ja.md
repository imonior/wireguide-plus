...
- **最小化で起動** — Windows では起動時にタスクバーへ最小化します（タスクバーのアイコンは
  ...
  最小化されます。
- **トレイ接続通知** — 起動15秒後（昇格プロンプトの確定後）に全トンネルの overview バブルを表示し、
  トンネルごとの最新状態と自動/手動モードを一覧します。表示後は設定可能な表示時間（既定10秒、設定で変更可）で自動的に閉じます。
  その後のネットワーク変更（Wi-Fi 切替・ケーブル抜去・ネットワーク喪失など）でトンネル状態が
  変わった場合は、トンネルごと1行ずつの状態バブル（「🟢 Connected: 名前」緑 /「🔴 Disconnected: 名前」赤 / 🟡 接続中は琥珀）を表示します。
  バブルはフォーカスを奪わず他のアプリの上に浮き、**ユーザーが操作するまで表示を維持**（閉じる・メインウィンドウを開く・切断、または新しいイベントに置換）します。
  バブルは作業領域の隅に停まりコンテンツを隠さず、ドラッグでどこへでも動かせます。
  3プラットフォームとも同一の挙動です（以前 macOS と Linux にはバブルがありませんでした）。
- **トレイ操作の統一** — 左クリックでも右クリックでもメニューを開き、左ダブルクリックでメインウィンドウを開きます（3プラットフォーム共通）。
  メニュー項目は macOS/Linux では 🟢 / 🔴 のステータス記号付き（Windows のメニューは GDI 描画でカラ―絵文字が使えないため ✔ / ✗）。
...
4. **システム統合** — ログイン時自動起動、最小化起動（Windows はタスクバー / macOS・
   Linux はトレイ）、トレイ接続通知（起動15秒後に全トンネル概要、表示時間は変更可；状態変更バブルは
   フォーカスを奪わず操作まで表示を維持）、トレイ操作の統一（クリックでメニュー、ダブルクリックでメインウィンドウ）。
...
- **SSID 不明時はフェイルクローズ**：現在のネットワークの SSIDが一時的に不明な場合——Wi-Fi インターフェースが揺らぎから先に復帰し、ID の再報告がまだ追いついていない状態（同一 SSID での再設定）——SSID 条件のルールは**判定不能**として扱われ、そのトンネルは今回の評価をスキップします。エンジンがまだ判定できない条件にデフォルト状態が黙って上書きすることはありません。SSID が再び判明した瞬間に評価も再開されます。
...
手動オーバーライドはアプリの再起動までしか続きません。自動化に**決して**動かされたくないトンネル——典型的には別の WireGuard クライアントも同じトンネルを管理している場合——には、トンネルの自動化エディターで「自動化」をオフにします。トンネルはルールとデフォルト状態を維持しますが、エンジンは接続も切断も行いません。

この設定はトンネル詳細ビューの上部にあるワンクリックの「自動化を停止」トグルでも行え、トンネルを見ているその場で自動化を抑制できます。グローバルな「システムのスリープを防止」設定（既定オフ）はトンネルが接続されている間 PC をスリープさせず（Windows・macOS・Linux のいずれでも有効）、最後のトンネルが切断されると即座に解除されるため、アイドル/スリープの電源ポリシーがトンネルを静かに切ることはありません。別のクライアントが既にトンネルのアドレスを占有している場合、WireGuide Plus は競合するソフトウェアを明示し、そのトンネルの自動化状態を表示する常駐ダイアログを表示します。選択肢は「そのトンネルの自動化を停止」と「再試行を継続」の二つだけで、決定するまでそのトンネルの自動接続は保留されます。未処理の衝突は GUI 起動時および helper 再起動後に再表示されます——決定するのはあなた、他のクライアントを強制停止することはありません。

### 1 つのトンネルに 1 つのクライアント

2 つの WireGuard クライアントが同じトンネルを同時に実行することはできません。トンネルの `Address` は先着順でいずれかのアダプターが占有し、後から接続したクライアントはアドレス割り当ての段階で失敗します（Windows は `The object already exists.` を報告）。公式 WireGuard クライアントも併用している場合は、同じトンネルのサービスを先に停止してください（例: `Stop-Service 'WireGuardTunnel$<トンネル名>'`）。あるいはそのトンネルを自動化から除外します（上記）。WireGuide Plus はこの衝突を検出し、競合するアダプターの名前を常駐衝突ダイアログと helper ログの両方に記録し、失敗した自動化接続をバックオフ付きで再試行します（30 秒 → 1 分 → 2 分 → 5 分。ポーリングのたびにアダプターに連続してぶつかることはありません）。
...
ログビューアのツールバーと設定のログセクションの両方に「ログフォルダを開く」ボタンがあり、システムファイラでこのフォルダをそのまま開けます。
