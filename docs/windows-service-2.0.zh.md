...
> 目标版本：2.0
> 范围：**特指 Windows 版本**。把 1.x 的「GUI 驱动 + 提权 helper」架构改造为「Windows 系统服务」架构。
...
>
> English version: see [windows-service-2.0.md](windows-service-2.0.md)

---

## 0. 范围：为什么特指 Windows

2.0 的系统服务化改造**仅针对 Windows**：
...
**2.0 中管理 GUI 仍是主要管理界面**，只是从服务的「所有者」变成服务的「客户端」：不再每次登录弹 UAC，托盘 + 设置 + 自动化交互全部保留。

## 1. 目标与收益

2.0 核心目标：核心引擎（隧道管理、防火墙、自动化）作为 **Windows 系统服务常驻运行**，开机自启、无人值守，有无 GUI 都正常工作。

| 收益 | 说明 |
|---|---|
| 开机即就绪 | 服务由 SCM 在登录前启动，无需登录/UAC 弹窗 |
| 无人值守 | WiFi 规则自动连接、断线重连、崩溃恢复在无 GUI 时照常工作 |
| 稳定性 | SCM 恢复策略（服务崩溃自动重启）比现 UAC 拉起更可靠 |
| Session 0 | 锁屏/注销后隧道不中断 |
| 解耦 | GUI 只是可替换客户端，可独立更新/关闭 |
...
1. 新增 `internal/service/service_windows.go`：
   - 实现 `svc.Handler`：`Execute(args, requests, changes)` 循环处理 `SVC_CONTROL_STOP` → 调用 `h.server.Shutdown()`（复用现有干净退出路径）；`SVC_CONTROL_INTERROGATE` → 报告 Running。
