...
- **"업데이트를 자동으로 조용히 설치" 설정 (설정 → 업데이트)** — 켜두면 다운로드된 업데이트가 설치 프로그램 UI 없이 설치된 후 앱이 자동으로 다시 시작됩니다. 기본값은 꺼짐입니다.
- **Windows 업데이트는 기본적으로 대화형 설치 프로그램 사용** — "지금 업데이트"로 다운로드가 완료되면 일반 설치와 동일한 설치 마법사가 실행되어 확인 후 설치할 수 있습니다. 완료 페이지(기본 체크된 "실행")에서 새 버전이 실행됩니다.

### 🐛 수정

- **Linux: 업데이트 완료 후 앱 자동 실행** — deb/rpm 패키지 관리자 업데이트 후 앱이 자동으로 다시 시작되어 새 버전이 로드됩니다 (이전에는 수동으로 열어야 했습니다).

> macOS는 1.4.0부터 도입된 앱 번들 인플레이스 업데이트를 유지합니다 (/Applications에 쓸 때 시스템 인증 프롬프트가 표시되며 완료 후 자동 재시작됩니다).
